# INDEX — Analyze-17

| Артефакт | Статус |
|----------|--------|
| [README.md](README.md) | done |
| [RULES.md](RULES.md) | done |
| [LIVING_SPEC_MATRIX.md](LIVING_SPEC_MATRIX.md) | done |
| [provenance/protocol-scope.md](provenance/protocol-scope.md) | done |
| [todo/TODO_FIXME.md](todo/TODO_FIXME.md) | done |
| [verification/CHECKLIST.md](verification/CHECKLIST.md) | done |

## Issues #71–#75

| # | Deliverable |
|---|-------------|
| 71 | Protocol Envelope / Response schemas + types |
| 72 | AIRA-EP Event Protocol adapter |
| 73 | AIRA-AP Artifact Protocol adapter |
| 74 | AIRA-ID Identity Descriptor |
| 75 | Local capability Discovery registry |

## Prev / Next
- Prev: [../Analyze-16/](../Analyze-16/) Epic 9 Conformance C0/C1
- Next: [../Analyze-18/](../Analyze-18/) Epic 11 MVP Alpha
