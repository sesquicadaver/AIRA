...
**Handoff:** Phase X first OPEN `#345` (після `#344` inventory DONE). Tip: `#342` DONE @ Analyze-379 / RFC-0215; `#343` DONE @ Analyze-380; `#344` DONE @ Analyze-381 / RFC-0227.
                  ...
                  → W1 reuse candidate (#340 DONE @ RFC-0224)
                    → W1 result/verify binding (#341 DONE @ RFC-0225)
                      → W close RFC-0215 (#342 DONE)
                        → Phase X Pack 2 (#343–#344 DONE; first OPEN `#345`)
```
...
**Статус фази:** **IN PROGRESS** @ RFC-0226 reserved (після **QUEUE W closed** @ RFC-0215). `#343`–`#344` DONE; **перший OPEN:** `#345`.
...
| 344 | **DONE** | ~~Analyze-381 — Per-model inventory lifecycle (M1)~~ | A/B independent; latest ≠ global auth; RFC-0227 | lifecycle green | select API (#345) |
| 345 | **OPEN** | Analyze-382 — Model select API Auto/required (M2) | Auto + required; unready explained; RFC-0228 | API green | profile (#346) |
...
**Перший OPEN:** `#345` (Model select API Auto/required). Tip: `#344` DONE @ Analyze-381 / RFC-0227.

```text
X0 wiring (#343 DONE)
  → X1 inventory M1 (#344 DONE)
    → X1 select API M2 (#345 OPEN / first OPEN)
      → X1 profile→snapshot M3 (#346)
...
| Pack 2 local multi-model GUI (aira-current 2026-09-13 + M1–M6) | [`docs/phase-x-plan.md`](docs/phase-x-plan.md); `#343`–`#358` **IN PROGRESS**; first OPEN `#345`; RFC-0226 reserved |
