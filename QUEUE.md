...
| # | Status | Analyze | Атомарний scope | Done when | Не в цьому рядку |
|---|--------|---------|-----------------|-----------|------------------|
| 275 | **DONE** | ~~Analyze-310 — Phase Q wiring~~ | plan + QUEUE `#275`–`#285` + `phase_q_doc`; RFC-0164 reserved; fmt tip | plan IN PROGRESS; `#275` DONE; first OPEN `#276` | identity (#276) |
| 276 | **OPEN** | Analyze-311 — Unique Desktop identity | install-scoped persistent ID; TrustStore collision explicit; two-root bootstrap test | runtime + тести + RFC-D | model observe (#277) |
...
**IN PROGRESS.** `#275` **DONE**. First OPEN: **`#276`** (first OPEN `#276`). Consolidating RFC-0164 file-free until `#285`.

```text
Q0 wiring (#275 DONE)
  → Q1 unique identity (#276 OPEN)
