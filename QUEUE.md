...
| Активна черга | Phase Z [`docs/phase-z-plan.md`](docs/phase-z-plan.md) `#390`–`#397` **IN PROGRESS**; **Перший OPEN:** `#391`. `#390` **DONE** @ Analyze-398 / `phase_z_doc`. Audit A `#376`–`#389` **DONE**. Phase Y `#359`–`#375` **DONE**. Phase X `#343`–`#358` **DONE** @ RFC-0226; **QUEUE X closed** |
...
6. **План-перед-чергою:** поодинокі пункти не додавати. Phase C `#38`–`#52` DONE. Phase D `#53`–`#74` DONE. Phase E `#75`–`#106` DONE. Phase F `#107`–`#119` DONE. Phase G — [`docs/phase-g-plan.md`](docs/phase-g-plan.md); `#120`–`#151` **DONE** (2026-08-28). Phase H — [`docs/phase-h-plan.md`](docs/phase-h-plan.md); `#152`–`#183` **DONE** (RFC-0077). Phase I — [`docs/phase-i-plan.md`](docs/phase-i-plan.md); `#184`–`#198` **DONE** (RFC-0078). Phase J — [`docs/phase-j-plan.md`](docs/phase-j-plan.md); `#199`–`#208` **DONE** (RFC-0096). Phase K — [`docs/phase-k-plan.md`](docs/phase-k-plan.md); `#209`–`#216` **DONE** (RFC-0104). Phase L — [`docs/phase-l-plan.md`](docs/phase-l-plan.md); `#217`–`#223` **DONE** @ RFC-0111. QUEUE L closed. Phase M — [`docs/phase-m-plan.md`](docs/phase-m-plan.md); `#224`–`#230` **DONE** @ RFC-0117. QUEUE M closed. Phase N — [`docs/phase-n-plan.md`](docs/phase-n-plan.md); `#231`–`#247` **DONE** @ RFC-0123; QUEUE N closed. Phase N-fix — [`docs/phase-n-fix-plan.md`](docs/phase-n-fix-plan.md); `#248`–`#254` **DONE** @ RFC-0139; QUEUE N-fix closed. Phase O — [`docs/phase-o-plan.md`](docs/phase-o-plan.md); `#255`–`#265` **DONE** @ RFC-0146; QUEUE O closed. Phase P — [`docs/phase-p-plan.md`](docs/phase-p-plan.md); `#266`–`#274` **DONE** @ RFC-0156; QUEUE P closed. Phase Q — [`docs/phase-q-plan.md`](docs/phase-q-plan.md); `#275`–`#285` **DONE** @ RFC-0164; **QUEUE Q closed**. Phase R — [`docs/phase-r-plan.md`](docs/phase-r-plan.md); `#286`–`#294` **DONE** @ RFC-0174; **QUEUE R closed**. Phase S — [`docs/phase-s-plan.md`](docs/phase-s-plan.md); `#295`–`#305` **DONE** @ RFC-0182; **QUEUE S closed**. Phase T — [`docs/phase-t-plan.md`](docs/phase-t-plan.md); `#306`–`#312` **DONE** @ RFC-0192; **QUEUE T closed**. Phase U — [`docs/phase-u-plan.md`](docs/phase-u-plan.md); `#313`–`#322` **DONE** @ RFC-0198; **QUEUE U closed**. Phase V — [`docs/phase-v-plan.md`](docs/phase-v-plan.md); `#323`–`#330` **IN PROGRESS** (Repair Pack 1; RFC-0208 reserved).
...
| 234 | **DONE** | ~~Analyze-269 — Presence Record~~ | schema + canonical Ed25519; mutation tests | schema + тести + RFC-0126 | Rendezvous trait (#235) |
| 235 | **DONE** | ~~Analyze-270 — RendezvousProvider~~ | trait API; no ledger logic in Core | trait + mock + RFC-0127 | EVM adapter (#236) |
| 236 | **DONE** | ~~Analyze-271 — EVM ledger adapter~~ | local deterministic double; Amoy/mainnet config hooks | adapter + тести + RFC-0128 | publish/query (#237) |
| 237 | **DONE** | ~~Analyze-272 — Publish/query~~ | publish/update/query_active/query_identity; TTL/sequence | тести + RFC-0129 | Reachability probe (#238) |
| 238 | **DONE** | ~~Analyze-273 — Reachability Probe~~ | peer-assisted signed challenge; no hairpin proof | тести + RFC-0130 | state machine (#239) |
| 239 | **DONE** | ~~Analyze-274 — Reachability states~~ | UNKNOWN…OFFLINE; `reachability.json` | тести + RFC-0131 | AddressBook (#240) |
| 240 | **DONE** | ~~Analyze-275 — AddressBook promotion~~ | valid Presence + trust policy → AddressBook only | тести + RFC-0132 | relay (#241) |
| 241 | **DONE** | ~~Analyze-276 — Relay integration~~ | direct→NAT→relay; prime relay ads; dual reservation | тести + RFC-0133 | refresh (#242) |
| 242 | **DONE** | ~~Analyze-277 — Presence refresh~~ | sequence++; expire stale; endpoint change | тести + RFC-0134 | CLI (#243) |
| 243 | **DONE** | ~~Analyze-278 — CLI~~ | `peer port` / `reachability` / `rendezvous` | CLI + тести + RFC-0135 | Desktop (#244) |
| 244 | **DONE** | ~~Analyze-279 — Desktop UX~~ | Network: port/reachability/rendezvous/peer count | GUI + тести + RFC-0136 | ab ovo (#245) |
| 245 | **DONE** | ~~Analyze-280 — Ab ovo integration~~ | A publish → B discover → trust → dial; no preconfigured peers | integration + RFC-0137 | NAT/relay (#246) |
| 246 | **DONE** | ~~Analyze-281 — NAT/relay integration~~ | both inbound blocked → relay Noise succeeds | integration + RFC-0138 | RFC close (#247) |
| 247 | **DONE** | ~~Analyze-282 — Phase N docs + RFC-0123~~ | consolidating RFC-0123; QUEUE N closed | QUEUE N DONE @ RFC-0123 | — |
...
**Наступне:** Phase O DONE @ RFC-0146; Phase P [`docs/phase-p-plan.md`](docs/phase-p-plan.md) `#266`–`#274`.

```text
N0 govern (#231)
  → N1 Prime Port (#232) DONE
    → N2 selection (#233) DONE
      → N3 Presence (#234) DONE
        → N4 RendezvousProvider (#235) DONE
          → N5 EVM adapter (#236) DONE
            → N6 publish/query (#237) DONE
              → N7 Reachability Probe (#238) DONE
                → N8 states (#239) DONE
                  → N9 AddressBook (#240) DONE
                    → N10 relay (#241) DONE
          → N7–N8 Reachability (#238–#239)
            → N9–N11 mesh glue (#240–#242) DONE
              → N12–N13 UX (#243–#244)
...
## Активна черга (лінійна) — Phase N-fix: Semantic honesty & live rendezvous

План: [`docs/phase-n-fix-plan.md`](docs/phase-n-fix-plan.md). Джерело: post-N audit `aira-current.md` @ `43bf3fa` (2026-09-06). QUEUE N closed @ RFC-0123.  
**Не** нова функціональна фаза; **не** GPU marketplace; **не** LLM у `aira-core`; consolidating RFC-0139 у `#254`.
...
| 250 | **DONE** | ~~Analyze-285 — Reachability session bind~~ | attestation requires inbound session/transcript | тести: no-connect ≠ DIRECT; RFC-0142 | expiry (#251) |
| 251 | **DONE** | ~~Analyze-286 — Expiry before promote~~ | expired Presence fail-closed even via `query_identity` | тести + docs; RFC-0143 | netns NAT (#252) |
| 252 | **DONE** | ~~Analyze-287 — netns NAT/firewall tests~~ | inbound block via ns/firewall; honest naming | integration + docs; RFC-0144 | port wrap (#253) |
| 253 | **DONE** | ~~Analyze-288 — `next_candidate_port` wrap~~ | modular ring arithmetic + wrap unit test | тести; RFC-0145 | Presence temporal (#254) |
| 254 | **DONE** | ~~Analyze-289 — Presence temporal + RFC-0139~~ | `created_at < expires_at` in validate_shape; honesty close | RFC-0139; QUEUE N-fix closed | — |
...
**Не** GPU marketplace; **не** LLM у `aira-core`. Consolidating [`AIRA-RFC-0146`](specs/rfc/AIRA-RFC-0146-phase-o-desktop-ux-help.md) @ `#265`.
...
| 256 | **DONE** | ~~Analyze-291 — SystemSnapshot honesty~~ | typed projection; UNKNOWN≠OFFLINE; book≠sessions | runtime + тести + RFC-0147 | non-blocking (#257) |
| 257 | **DONE** | ~~Analyze-292 — Non-blocking work/refresh~~ | submit/refresh off egui `update()`; repaint≠data | async_jobs + тести + RFC-0148 | IDs (#258) |
| 258 | **DONE** | ~~Analyze-293 — Action/error/help IDs~~ | stable IDs + Labels lexicon map | lexicon + тести + RFC-0149 | shell IA (#259) |
| 259 | **DONE** | ~~Analyze-294 — Shell IA~~ | Work / System / Settings + Help·F1 chrome | GUI + docs + RFC-0150 | Work body (#260) |
| 260 | **DONE** | ~~Analyze-295 — Work screen~~ | user-language; draft; Ctrl+Enter; provenance | GUI + тести + RFC-0151 | status (#261) |
| 261 | **DONE** | ~~Analyze-296 — System status screen~~ | Program/Model/Connection/Events | GUI + тести + RFC-0152 | settings (#262) |
| 262 | **DONE** | ~~Analyze-297 — Settings apply lifecycle~~ | Saved≠Applied≠Restart needed | GUI + тести + RFC-0153 | F1 (#263) |
| 263 | **DONE** | ~~Analyze-298 — Offline F1 shell~~ | embed help; search; context routing; offline | help + GUI + тести + RFC-0154 | topics (#264) |
| 264 | **DONE** | ~~Analyze-299 — Seed help topics~~ | uk/en catalog IDs; link check | `docs/help/**` + тести + RFC-0155 | RFC close (#265) |
| 265 | **DONE** | ~~Analyze-300 — RFC-0146 + close~~ | consolidating RFC-0146; QUEUE O closed | RFC-0146; QUEUE O closed | — |

### Наступний цикл (Phase O)

**Phase O COMPLETE** (`#255`–`#265` **DONE** @ RFC-0146). **QUEUE O closed.** no OPEN O atoms.  
**Наступне:** Phase P [`docs/phase-p-plan.md`](docs/phase-p-plan.md) `#266`–`#274`.

```text
O0 wiring (#255 DONE)
  → O1 snapshot (#256 DONE @ RFC-0147)
    → O2 non-blocking (#257 DONE @ RFC-0148)
      → O3 IDs (#258 DONE @ RFC-0149)
        → O4 shell (#259 DONE @ RFC-0150)
          → O5 work (#260 DONE @ RFC-0151)
            → O6 status (#261 DONE @ RFC-0152)
              → O7 settings (#262 DONE @ RFC-0153)
                → O8 F1 (#263 DONE @ RFC-0154)
                  → O9 topics (#264 DONE @ RFC-0155)
                    → O10 RFC-0146 (#265 DONE)
```
...
## Активна черга (лінійна) — Phase P: Runtime-honest Desktop (post-O)

План: [`docs/phase-p-plan.md`](docs/phase-p-plan.md). Канон: [`docs/desktop-ux.md`](docs/desktop-ux.md). Джерело: post-O audit `aira-current.md` @ `700ffd5` (2026-09-07). QUEUE O closed @ RFC-0146.  
**Не** GPU marketplace; **не** LLM у `aira-core`; consolidating RFC-0156 у `#274` (file at close).

| # | Status | Analyze | Атомарний scope | Done when | Не в цьому рядку |
|---|--------|---------|-----------------|-----------|------------------|
| 266 | **DONE** | ~~Analyze-301 — Phase P wiring~~ | plan + QUEUE `#266`–`#274` + `phase_p_doc`; RFC-0156 reserved | plan IN PROGRESS; `#266` DONE; first OPEN `#267` | freshness (#267) |
| 267 | **DONE** | ~~Analyze-302 — Snapshot freshness~~ | Stale path; measurement≠load; bind≠listener | runtime + тести + RFC-0157 | applied (#268) |
| 268 | **DONE** | ~~Analyze-303 — Applied from runtime~~ | applied from confirmed runtime; Undefined | GUI + тести + RFC-0158 | model (#269) |
| 269 | **DONE** | ~~Analyze-304 — Model triple UX~~ | selected≠ready≠used | GUI + тести + RFC-0159 | reachability (#270) |
| 270 | **DONE** | ~~Analyze-305 — Reachability endpoint bind~~ | endpoint+direction; CLI transcript alone ≠ verified | peer + тести + RFC-0160 | EVM honesty (#271) |
| 271 | **DONE** | ~~Analyze-306 — EVM/JSON-RPC honesty~~ | https Amoy path; Mock ≠ ledger claim | adapter + docs + тести + RFC-0161 | lifecycle (#272) |
| 272 | **DONE** | ~~Analyze-307 — Lifecycle non-blocking~~ | Start/Stop/Quit off update(); refresh gen invalidate | async_jobs + тести + RFC-0162 | Help (#273) |
| 273 | **DONE** | ~~Analyze-308 — Help F1 routing~~ | clear search; focus→section→screen; titles | GUI + тести + RFC-0163 | RFC close (#274) |
| 274 | **DONE** | ~~Analyze-309 — RFC-0156 + close~~ | consolidating RFC-0156; QUEUE P closed | RFC-0156; QUEUE P closed | — |

### Наступний цикл (Phase P)

**Phase P COMPLETE** (`#266`–`#274` **DONE** @ RFC-0156). **QUEUE P closed.** no OPEN P atoms.

**Наступне (історично):** Phase Q [`docs/phase-q-plan.md`](docs/phase-q-plan.md) `#275`–`#285` **DONE** @ RFC-0164; QUEUE Q closed.


---

## Активна черга (лінійна) — Phase Q: Cross-install integrity & light monitoring (post-P)

План: [`docs/phase-q-plan.md`](docs/phase-q-plan.md). Канон: [`docs/desktop-ux.md`](docs/desktop-ux.md). Джерело: post-P audit `aira-current.md` @ `00f19cf` (2026-09-07). QUEUE P closed @ RFC-0156.  
**Не** GPU marketplace; **не** LLM у `aira-core`; consolidating RFC-0164 у `#285` **DONE**.

| # | Status | Analyze | Атомарний scope | Done when | Не в цьому рядку |
|---|--------|---------|-----------------|-----------|------------------|
| 275 | **DONE** | ~~Analyze-310 — Phase Q wiring~~ | plan + QUEUE `#275`–`#285` + `phase_q_doc`; RFC-0164 reserved; fmt tip | plan DONE @ RFC-0164; QUEUE Q closed | identity (#276) |
| 276 | **DONE** | ~~Analyze-311 — Unique Desktop identity~~ | install-scoped persistent ID; TrustStore collision explicit; two-root bootstrap test | runtime + тести + RFC-0165 | model observe (#277) |
| 277 | **DONE** | ~~Analyze-312 — Model light observe~~ | no full weights hash on status refresh; cached ready; admit still full-hashes | runtime + тести + RFC-0166 | verify context (#278) |
| 278 | **DONE** | ~~Analyze-313 — Model verify context~~ | root-scoped verify; reopen without test keyring | runtime + тести + RFC-0167 | reach obs (#279) |
| 279 | **DONE** | ~~Analyze-314 — Reachability observation independence~~ | bind ≠ external freshness; skew bound | peer + тести + RFC-0168 | NAT (#280) |
| 280 | **DONE** | ~~Analyze-315 — Reachability NAT endpoints~~ | dial≠accepted local_addr one challenge | peer + тести + RFC-0169 | evidence (#281) |
| 281 | **DONE** | ~~Analyze-316 — Reachability evidence admission~~ | root-bound; replay; apply-time freshness | peer/CLI + тести + RFC-0170 | lifecycle (#282) |
| 282 | **DONE** | ~~Analyze-317 — Lifecycle revision races~~ | one revision; Quit-during-Start; Applied from worker | desktop + тести + RFC-0171 | provenance (#283) |
| 283 | **DONE** | ~~Analyze-318 — Backend≠model used~~ | used model ≠ backend id; undefined without evidence | execution/desktop + тести + RFC-0172 | invite (#284) |
| 284 | **DONE** | ~~Analyze-319 — Invite import atomicity~~ | checks before TrustStore/book writes; commit/rollback | desktop + тести + RFC-0173 | RFC close (#285) |
| 285 | **DONE** | ~~Analyze-320 — RFC-0164 + close~~ | consolidating RFC-0164; QUEUE Q closed | RFC-0164; QUEUE Q closed | — |

### Наступний цикл (Phase Q)

**Phase Q COMPLETE** (`#275`–`#285` **DONE** @ RFC-0164). **QUEUE Q closed.** no OPEN Q atoms. A next phase is not activated here.


---

## Архів (закрито) — Phase R: Actionable Desktop Connection UX (post-Q)

План: [`docs/phase-r-plan.md`](docs/phase-r-plan.md). Канон: [`docs/desktop-ux.md`](docs/desktop-ux.md). QUEUE Q closed @ RFC-0164.  
**Не** GPU marketplace; **не** LLM у `aira-core`; consolidating RFC-0174 у `#294` **DONE**.

| # | Status | Analyze | Атомарний scope | Done when | Не в цьому рядку |
|---|--------|---------|-----------------|-----------|------------------|
| 286 | **DONE** | ~~Analyze-322 — Phase R wiring~~ | plan + QUEUE `#286`–`#294` + `phase_r_doc`; RFC-0174 reserved; fmt tip | plan IN PROGRESS; `#286` DONE; first OPEN `#287` | CTA (#287) |
| 287 | **DONE** | ~~Analyze-323 — Connection next-step CTA~~ | стан → один primary CTA (`connection_cta` + UI); RFC-0175 | `#287` DONE; first OPEN `#288` | promote (#288) |
...
| 293 | **DONE** | ~~Analyze-329 — Cold-start empty profile~~ | guidance + CTA; RFC-0181 | `#293` DONE; tip `#294` close | RFC close (#294) |
| 294 | **DONE** | ~~Analyze-330 — RFC-0174 + close~~ | consolidating RFC-0174; QUEUE R closed | RFC-0174; QUEUE R closed | — |

### Наступний цикл (Phase R)

**Phase R COMPLETE** (`#286`–`#294` **DONE** @ RFC-0174). **QUEUE R closed.** no OPEN R atoms. A next phase is not activated here.

```text
R0 wiring (#286 DONE)
  → R1 Connection CTA (#287 DONE @ RFC-0175)
    → R2 promote connect controls (#288 DONE @ RFC-0176)
      → R3 human-primary language (#289 DONE @ RFC-0177)
        → R4 problem/strip actions (#290 DONE @ RFC-0178)
          → R5 Help scenario (#291 DONE @ RFC-0179)
            → R6 UK parity (#292 DONE @ RFC-0180)
              → R7 cold-start (#293 DONE @ RFC-0181)
                → R close RFC-0174 (#294 DONE)
```

Анти-місія README **ніколи** в QUEUE.


**Наступне:** Phase T активовано — див. секцію Phase T вище.

---

## Активна черга (лінійна) — Phase S: Cross-boundary integrity & connect honesty (post-R)

План: [`docs/phase-s-plan.md`](docs/phase-s-plan.md). Канон: [`docs/desktop-ux.md`](docs/desktop-ux.md). QUEUE R closed @ RFC-0174.  
**Не** GPU marketplace; **не** LLM у `aira-core`; consolidating RFC-0182 у `#305` **DONE**; **не** reachability key-bound (`#296`) у wiring.

| # | Status | Analyze | Атомарний scope | Done when | Не в цьому рядку |
|---|--------|---------|-----------------|-----------|------------------|
| 295 | **DONE** | ~~Analyze-331 — Phase S wiring~~ | plan + QUEUE `#295`–`#305` + `phase_s_doc`; RFC-0182 reserved | plan IN PROGRESS; `#295` DONE; first OPEN `#296` | reach key (#296) |
| 296 | **DONE** | ~~Analyze-332 — Reachability key-bound admission~~ | same-ID / foreign-key import → reject; verify vs root keyring; RFC-0183 | `#296` DONE; first OPEN `#297` | model reopen (#297) |
| 297 | **DONE** | ~~Analyze-333 — Model descriptor root-scoped verify~~ | descriptor+payload verify після cold reopen; RFC-0184 | `#297` DONE; first OPEN `#298` | identity pair (#298) |
| 298 | **DONE** | ~~Analyze-334 — Identity incomplete-pair fail-closed~~ | mint лише якщо обох немає; partial → error; RFC-0185 | `#298` DONE; first OPEN `#299` | Help boundary (#299) |
| 299 | **DONE** | ~~Analyze-335 — Connect Help boundary honesty~~ | Help/CTA EN+UK: setup ≠ remote; loopback ≠ dial; RFC-0186 | `#299` DONE; first OPEN `#300` | dial path (#300) |
| 300 | **DONE** | ~~Analyze-336 — Opt-in peer dial / session evidence~~ | явна адреса + dial + evidence; RFC-0187 | `#300` DONE; first OPEN `#301` | CTA Stop (#301) |
| 301 | **DONE** | ~~Analyze-337 — CTA Stop→Start matrix~~ | stopped → Start not Refresh-only; RFC-0188 | `#301` DONE; first OPEN `#302` | submit gate (#302) |
| 302 | **DONE** | ~~Analyze-338 — Submit∥Start/Stop admission~~ | submit⊥lifecycle; no double start; RFC-0189 | `#302` DONE; first OPEN `#303` | observe UI (#303) |
| 303 | **DONE** | ~~Analyze-339 — Observe miss off UI-thread~~ | UI observe miss → background streaming hash; no full buffer read; admit full-hash; RFC-0190 | `#303` DONE; first OPEN `#304` | durability (#304) |
| 304 | **DONE** | ~~Analyze-340 — Reachability durability honesty~~ | docs: replay≠state joint atomic; RFC-0191 | `#304` DONE; tip was `#305` | RFC close (#305) |
| 305 | **DONE** | ~~Analyze-341 — RFC-0182 + close~~ | consolidating RFC-0182; QUEUE S closed | RFC-0182; QUEUE S closed | — |

### Наступний цикл (Phase S)

**Phase S COMPLETE** (`#295`–`#305` **DONE** @ RFC-0182). **QUEUE S closed.** no OPEN S atoms. Phase T активовано — див. секцію Phase T нижче.

```text
S0 wiring (#295 DONE)
  → S1 reachability key-bound (#296 DONE @ RFC-0183)
    → S1 model descriptor reopen (#297 DONE @ RFC-0184)
      → S1 identity incomplete-pair (#298 DONE @ RFC-0185)
        → S1 Help connect boundary (#299 DONE @ RFC-0186)
          → S1 opt-in dial/session (#300 DONE @ RFC-0187)
            → S2 CTA Stop→Start (#301 DONE @ RFC-0188)
              → S2 submit∥lifecycle (#302 DONE @ RFC-0189)
                → S2 observe off-UI (#303 DONE @ RFC-0190)
                  → S2 durability honesty (#304 DONE @ RFC-0191)
                    → S close RFC-0182 (#305 DONE)
```
...
## Активна черга (лінійна) — Phase T: Operation lifecycle honesty (post-S)

План: [`docs/phase-t-plan.md`](docs/phase-t-plan.md). Канон: [`docs/desktop-ux.md`](docs/desktop-ux.md). QUEUE S closed @ RFC-0182.  
**Не** GPU marketplace; **не** LLM у `aira-core`; consolidating RFC-0192 у `#312` **DONE**; **не** dial live-honesty (`#307`) у wiring.

| # | Status | Analyze | Атомарний scope | Done when | Не в цьому рядку |
|---|--------|---------|-----------------|-----------|------------------|
| 306 | **DONE** | ~~Analyze-342 — Phase T wiring~~ | plan + QUEUE `#306`–`#312` + `phase_t_doc`; RFC-0192 reserved | plan IN PROGRESS; `#306` DONE; first OPEN `#307` | dial live (#307) |
| 307 | **DONE** | ~~Analyze-343 — Dial evidence ≠ live session~~ | evidence ≠ live_session_count; last handshake only; RFC-0193 | `#307` DONE; first OPEN `#308` | dial async (#308) |
| 308 | **DONE** | ~~Analyze-344 — Opt-in dial off UI-thread~~ | dial via jobs/slot; no UI block_on; F1/nav; RFC-0194 | `#308` DONE; first OPEN `#309` | observe fail (#309) |
| 309 | **DONE** | ~~Analyze-345 — Observe miss fail durable~~ | Pending/Ready/Failed; no rehash storm; RFC-0195 | `#309` DONE; first OPEN `#310` | Quit∥Submit (#310) |
| 310 | **DONE** | ~~Analyze-346 — Quit∥Submit completion~~ | deferred Stop→Close after submit; RFC-0196 | `#310` DONE; first OPEN `#311` | AddressBook (#311) |
| 311 | **DONE** | ~~Analyze-347 — Dial AddressBook candidate honesty~~ | failed dial rollback book; RFC-0197 | `#311` DONE; tip was `#312` | RFC close (#312) |
| 312 | **DONE** | ~~Analyze-348 — RFC-0192 + close~~ | consolidating RFC-0192; QUEUE T closed | RFC-0192; QUEUE T closed | — |

### Наступний цикл (Phase T)

**Phase T COMPLETE** (`#306`–`#312` **DONE** @ RFC-0192). **QUEUE T closed.** no OPEN T atoms. Phase U активовано — див. секцію Phase U нижче.

```text
T0 wiring (#306 DONE)
  → T1 dial evidence ≠ live (#307 DONE @ RFC-0193)
    → T1 dial off-UI (#308 DONE @ RFC-0194)
      → T1 observe fail durable (#309 DONE @ RFC-0195)
        → T2 Quit∥Submit (#310 DONE @ RFC-0196)
          → T2 AddressBook candidate (#311 DONE @ RFC-0197)
            → T close RFC-0192 (#312 DONE)
```
...
## Архів (закрито) — Phase U: Cross-path contract honesty (post-T)

План: [`docs/phase-u-plan.md`](docs/phase-u-plan.md). Канон: [`docs/desktop-ux.md`](docs/desktop-ux.md). QUEUE T closed @ RFC-0192.
**Не** GPU marketplace; **не** LLM у `aira-core`; consolidating RFC-0198 у `#322` **DONE**; **не** Verification capsule (`#314`) у wiring.
Audit: `aira-current.md` @ `ef5f69c`.

| # | Status | Analyze | Атомарний scope | Done when | Не в цьому рядку |
|---|--------|---------|-----------------|-----------|------------------|
| 313 | **DONE** | ~~Analyze-349 — Phase U wiring~~ | plan + QUEUE `#313`–`#322` + `phase_u_doc`; RFC-0198 reserved | plan IN PROGRESS; `#313` DONE; first OPEN `#314` | verify (#314) |
| 314 | **DONE** | ~~Analyze-350 — Verification capsule-sourced~~ | capsule-sourced; substituted output fails; RFC-0199 | `#314` DONE; first OPEN `#315` | result (#315) |
| 315 | **DONE** | ~~Analyze-351 — Result-by-problem authority~~ | problem_id → ArtifactStore verify; index locator; RFC-0200 | `#315` DONE; first OPEN `#316` | identity (#316) |
| 316 | **DONE** | ~~Analyze-352 — CLI identity create fail-closed~~ | shared create; повторний create = error; RFC-0201 | `#316` DONE; first OPEN `#317` | policy (#317) |
| 317 | **DONE** | ~~Analyze-353 — Policy audit uniqueness~~ | унікальний ID; same-ID≠hash → conflict; RFC-0202 | `#317` DONE; first OPEN `#318` | failed submit (#318) |
| 318 | **DONE** | ~~Analyze-354 — Failed submit durable~~ | Err після accept → durable failure history; RFC-0203 | `#318` DONE; first OPEN `#319` | executor (#319) |
| 319 | **DONE** | ~~Analyze-355 — Submit executor honesty~~ | mock vs configured explicit; RFC-0204 | `#319` DONE; first OPEN `#320` | AddressBook (#320) |
| 320 | **DONE** | ~~Analyze-356 — AddressBook selective rollback~~ | rollback лише власний candidate; RFC-0205 | `#320` DONE; first OPEN `#321` | docs (#321) |
| 321 | **DONE** | ~~Analyze-357 — systemd/docs prime-port~~ | runbook + unit pass validate_aira_port; RFC-0206 | `#321` DONE; tip was `#322` | RFC close (#322) |
| 322 | **DONE** | ~~Analyze-358 — RFC-0198 + close~~ | consolidating RFC-0198; QUEUE U closed | RFC-0198; QUEUE U closed | — |

### Наступний цикл (Phase U)

**Phase U COMPLETE** (`#313`–`#322` **DONE** @ RFC-0198). **QUEUE U closed.** no OPEN U atoms. Deferred §8/§10 audit items remain out of U. Phase V активовано — див. секцію Phase V нижче.

```text
U0 wiring (#313 DONE)
  → U1 verify capsule-sourced (#314 DONE @ RFC-0199)
    → U1 result authority (#315 DONE @ RFC-0200)
      → U1 CLI identity (#316 DONE @ RFC-0201)
        → U1 policy audit IDs (#317 DONE @ RFC-0202)
          → U1 failed submit durable (#318 DONE @ RFC-0203)
            → U2 executor honesty (#319 DONE @ RFC-0204)
              → U2 AddressBook selective rollback (#320 DONE @ RFC-0205)
                → U2 systemd prime-port (#321 DONE @ RFC-0206)
                  → U close RFC-0198 (#322 DONE)
```
...
**Phase V COMPLETE** (`#323`–`#330` **DONE** @ RFC-0208). **QUEUE V closed.** no OPEN V atoms. Pack 2 GUI remains out of V. Residual honesty → Phase W нижче.
...
## Активна черга (лінійна) — Phase W: Pack 1 residual honesty

План: [`docs/phase-w-plan.md`](docs/phase-w-plan.md). Pack honesty: [`docs/repair-package-1.md`](docs/repair-package-1.md). Канон: [`docs/desktop-ux.md`](docs/desktop-ux.md) + [`docs/product-requirements-trace.md`](docs/product-requirements-trace.md). QUEUE V closed @ RFC-0208; audit `d1115f2`.
**Не** GPU marketplace; **не** LLM у `aira-core`; consolidating RFC-0215 у `#342` reserved file-free; **не** math capsule (`#332`) у wiring; **не** Pack 2 M1–M6.
Audit source: `p-only/AIRA-audit-d1115f2-2026-09-11.md`.
...
| 333 | **DONE** | ~~Analyze-370 — Strict submit decoding~~ | unknown nested fields → 4xx; RFC-0217 | `#333` DONE; first OPEN `#334` | constraints (#334) |
| 334 | **DONE** | ~~Analyze-371 — Constraints enforce-or-reject~~ | enforced or explicit unsupported; RFC-0218 | `#334` DONE; first OPEN `#335` | admission (#335) |
| 335 | **DONE** | ~~Analyze-372 — Admission boundary verify~~ | text/hash/kind mismatch reject; RFC-0219 | `#335` DONE; first OPEN `#336` | activate evidence (#336) |
...
| 340 | **DONE** | ~~Analyze-377 — Reuse candidate independent check~~ | foreign VRA ≠ Completed; RFC-0224 | `#340` DONE; first OPEN `#341` | result binding (#341) |
| 341 | **DONE** | ~~Analyze-378 — Result/verify task binding~~ | swap refs reject; generate honesty; RFC-0225 | `#341` DONE; first OPEN `#342` | RFC close (#342) |
| 342 | **DONE** | ~~Analyze-379 — RFC-0215 + close~~ | consolidating RFC-0215; QUEUE W closed | `#342` DONE; **QUEUE W closed**; first OPEN `#343` | Phase X `#343` |

### Наступний цикл (Phase W)

**Phase W COMPLETE** (`#331`–`#342` **DONE** @ RFC-0215). **QUEUE W closed.** no OPEN W atoms.  
**Handoff:** Phase X **DONE** @ RFC-0226; **QUEUE X closed**; no OPEN X atoms. Tip: `#342` DONE @ Analyze-379 / RFC-0215; `#343`–`#358` DONE @ Analyze-380…395 / RFC-0227…0240 + consolidating RFC-0226.

```text
W0 wiring + A0 (#331 DONE)
  → W1 math capsule (#332 DONE @ RFC-0216)
    → W1 strict submit (#333 DONE @ RFC-0217)
      → W1 constraints enforce-or-reject (#334 DONE @ RFC-0218)
        → W1 admission boundary (#335 DONE @ RFC-0219)
            ...
            → W1 production trust (#337 DONE @ RFC-0221)
              → W1 safe materialize (#338 DONE @ RFC-0222)
                → W1 backend binding (#339 DONE @ RFC-0223)
                  → W1 reuse candidate (#340 DONE @ RFC-0224)
                    → W1 result/verify binding (#341 DONE @ RFC-0225)
                      → W close RFC-0215 (#342 DONE)
                        → Phase X Pack 2 (#343–#358 DONE @ RFC-0226; QUEUE X closed)
```
...
## Активна черга (лінійна) — Phase X: Pack 2 local multi-model GUI
...
| # | Status | Analyze | Атомарний scope | Done when | Не в цьому рядку |
|---|--------|---------|-----------------|-----------|------------------|
| 343 | **DONE** | ~~Analyze-380 — Phase X wiring~~ | plan IN PROGRESS + QUEUE tip + `phase_x_doc`; RFC-0226 reserved | `#343` DONE; first OPEN `#344` | inventory (#344) |
| 344 | **DONE** | ~~Analyze-381 — Per-model inventory lifecycle (M1)~~ | A/B independent; latest ≠ global auth; RFC-0227 | `#344` DONE; first OPEN `#345` | select API (#345) |
| 345 | **DONE** | ~~Analyze-382 — Model select API Auto/required (M2)~~ | Auto + required; unready explained; RFC-0228 | `#345` DONE; first OPEN `#346` | profile (#346) |
| 346 | **DONE** | ~~Analyze-383 — Request profile → snapshot (M3)~~ | overrides/excludes; mid-flight safe; RFC-0229 | `#346` DONE; first OPEN `#347` | CLI (#347) |
| 347 | **DONE** | ~~Analyze-384 — CLI supported-contract parity (M4)~~ | no no-op flags; RFC-0230 | `#347` DONE; first OPEN `#348` | Settings GUI (#348) |
| 348 | **DONE** | ~~Analyze-385 — Settings Models catalog GUI~~ | list/scan/add/select/prepare + ready reason; RFC-0231 | `#348` DONE; first OPEN `#349` | Work executor (#349) |
| 349 | **DONE** | ~~Analyze-386 — Work executor + capability readiness~~ | Auto/specific; math≠generate; pre-submit; RFC-0232 | `#349` DONE; first OPEN `#350` | mock/result (#350) |
| 350 | **DONE** | ~~Analyze-387 — Mock honesty + result triple~~ | demo banner; requested/applied/executed; RFC-0233 | `#350` DONE; first OPEN `#351` | network UX (#351) |
| 351 | **DONE** | ~~Analyze-388 — Network/address honesty + P UX~~ | loopback; HTTP≠peer≠advertised; P names; P3\|P4 radio; RFC-0234 | `#351` DONE; first OPEN `#352` | copy/IA (#352) |
| 352 | **DONE** | ~~Analyze-389 — Human copy + Settings≠System IA~~ | i18n; program buttons; edit≠observe; RFC-0235 | `#352` DONE; first OPEN `#353` | Help (#353) |
| 353 | **DONE** | ~~Analyze-390 — Help F1 model path~~ | UK/EN offline model path; RFC-0236 | `#353` DONE; first OPEN `#354` | Compare (#354) |
...
## Phase Y — вибір моделі (OPEN; хвіст після K)

План Y: [`docs/phase-y-plan.md`](docs/phase-y-plan.md).  
План аудиту / gate: [`docs/audit-6007442-plan.md`](docs/audit-6007442-plan.md).  
Phase Y `#359`–`#375` **DONE**. Audit gate K `#376`–`#382` **DONE**. D+A `#383`–`#389` **DONE**. Live tip → Phase Z **перший OPEN `#391`** ([`phase-z-plan.md`](docs/phase-z-plan.md)). Не Pack 3.
...
| 362 | **DONE** | — | ~~«Підготувати й виконати»: слот → допуск → запуск; повтор без другого запуску; помилка не змінює tip~~ | тест кнопки і повтору | метадані, фільтр |
| 363 | **DONE** | — | ~~Екранний підпис не стає CLI-ім’ям; без точного імені bind заборонений~~ | тест відмови bind | перший рядок списку |

### Audit gate K — перед `#364` (**DONE**)

Джерело: аудит `6007442` (2026-09-23). Номери `#376`–`#382` у кінці хвоста; виконання **перед** відновленням `#364`.
...
| 379 | **DONE** | — | ~~CLI-ім’я лише з перевіреної прив’язки; `host_ollama_model_ref(cli)==model_ref`~~ | пошкоджений pointer видимий; Make default не створює bind B | `#364` first-row |
| 380 | **DONE** | — | ~~Каталог у фоновому snapshot; render без O(N×S) I/O на кадр~~ | Work/Settings не читають tip/cache на кожен paint | метадані HTTP |
| 381 | **DONE** | — | ~~Prepare-and-run = один async job (slot → admit → submit), один inflight~~ | UI не блокується; повтор без другого запуску | фільтр списку |
| 382 | **DONE** | — | ~~Одна оцінка **applied** executor + усі вибрані моделі для Prepare-and-run~~ | Mock≠Process і Compare узгоджені з Run; негативний тест кнопки→submit | повний `#367` E2E |

### Phase Y — хвіст (OPEN після K)

| # | Status | Analyze | Атомарний scope | Done when | Не в цьому рядку |
|---|--------|---------|-----------------|-----------|------------------|
| 364 | **DONE** | — | ~~«Використовувати Ollama» не бере перше ім’я списку~~ | тест resolver | endpoint |
| 365 | **DONE** | — | ~~Помилка оновлення зберігає знімок; успішний порожній список — «відсутня»~~ | два тести станів | HTTP show |
| 366 | **DONE** | — | ~~Той самий явний endpoint для discovery і дочірнього `ollama run`~~ | тест: успадкований `OLLAMA_HOST` не розщеплює список і запуск | `/api/tags` |
| 367 | **DONE** | — | ~~Наскрізний тест: A типова → B зі списку → підготовка → запуск → A типова (+ негативи K3–K5)~~ | тест включає помилку підготовки і повтор | метадані |
| 368 | **DONE** | — | ~~`/api/tags` у desktop-runtime; відсутнє поле = невідомо~~ | тест парсера без вигаданих `false` | `/api/show` |
| 369 | **DONE** | — | ~~`/api/show`, коли бракує будь-якого поля оцінки~~ | тест: capabilities є, місце немає → show | кеш |
| 370 | **DONE** | — | ~~Кеш (endpoint, ім’я, digest); тайм-аут; паралельність 2–4; старе покоління відкидається~~ | тести покоління і скидання endpoint | оцінка придатності |
| 371 | **DONE** | — | ~~Місце: локально щодо сервера vs цей комп’ютер~~ | тест: порожнє поле невідомої версії, `localhost` і `:cloud` не підтверджують локальність | селектор |
| 372 | **DONE** | — | ~~Одна оцінка для рядка, фільтра, кнопки, скорочення і перевірки перед запуском~~ | тест п’яти станів; `completion+vision` готова за решти умов | верстка пошуку |
| 373 | **DONE** | — | ~~Пошук, «Сумісні», «Показати всі», лічильник N із M; вибір за ID переживає фільтр~~ | тест лічильника і стійкого вибору | Compare-вхід |
| 374 | **DONE** | — | ~~Compare: B спочатку порожня; вихід повертає попередній вибір; різні повні імена не зливаються~~ | тест входу і виходу | F1 |
| 375 | **DONE** | — | ~~EN/UK і F1 для нових станів; рядок у `docs/local-node.md`~~ | допомога згадує «Підготувати й виконати» і не обіцяє локальність за суфіксом | нові правила оцінки |

### Docs D + acceptance A (OPEN після `#375`)

План: [`docs/audit-6007442-plan.md`](docs/audit-6007442-plan.md). Не стартують раніше `#375`.
...
| 384 | **DONE** | — | ~~C1 Executed vs VRA / OP-001 / matrix: три окремі статуси~~ | без змішування smoke і verified pipeline | калькулятор у GUI |
| 385 | **DONE** | — | ~~Settlement receipt: один `$id` ↔ одна schema~~ | JSON і schema-pack збігаються | federation runtime |
| 386 | **DONE** | — | ~~`specs/README`: baseline + amendments + пріоритет; RFC metadata~~ | без другого канону | перепис Manifesto |
| 387 | **DONE** | — | ~~Entry-points і зламані links~~ | одне актуальне «перше OPEN» | масове DONE без E2E |
| 388 | **DONE** | — | ~~Installed acceptance на конкретному SHA після `#367`+K~~ | GUI+CLI+HTTP; негативи; SHA/Ollama/digest | Analyze-396 як proof HEAD |
| 389 | **DONE** | — | ~~Старі smoke/Analyze-396 — історичні snapshots~~ | явна дата/SHA в шапці evidence | нові продуктові можливості |
...
**Перший OPEN:** `#391`. `#390` **DONE** @ Analyze-398. Audit A `#376`–`#389` **DONE** (closed provenance).
...
| 391 | **OPEN** | — | Work layout budget: editor `desired_rows` ≥ 8 або пропорційний split | тест/константа; answer не нижче мінімуму | remove how-it-works (#392) |
| 392 | **OPEN** | — | Прибрати «Як це працює» CollapsingHeader; навчання через F1 | немає `work_how_it_works` primary dump | Connection copy |
...
| 397 | **OPEN** | — | RFC-0244 consolidating + phase complete | tip → немає OPEN Z atoms | Pack 3+ |

### Наступний цикл

**Перший OPEN:** `#391` (Work layout budget). Phase Z `#390` **DONE** @ Analyze-398 / `phase_z_doc`. `#392`–`#397` **OPEN**. Audit A `#376`–`#389` **DONE**.

```text
#359 readiness … #389 Analyze-396 historicize (DONE; audit A closed)
  → #390 Phase Z wiring (DONE @ Analyze-398)
    → #391 Work layout budget (OPEN)
      → #392 Remove how-it-works
        → #393 Short sys_conn_unknown
...
Tip (historical Pack 2): `#358` DONE @ Analyze-395 / RFC-0226. Live tip → Phase Z **перший OPEN `#391`**. Pack 3 лишається окремо.

```text
X0 wiring (#343 DONE)
  → X1 inventory M1 (#344 DONE)
    → X1 select API M2 (#345 DONE)
      → X1 profile→snapshot M3 (#346 DONE)
        → X1 CLI parity M4 (#347 DONE)
          → X2 Settings Models (#348 DONE)
            → X2 Work executor (#349 DONE)
              → X2 mock + result (#350 DONE)
                → X2 network/P UX (#351 DONE @ RFC-0234)
                  → X2 copy + IA (#352 DONE @ RFC-0235)
                    → X2 Help F1 (#353 DONE @ RFC-0236)
                      → X2 Compare (#354 DONE @ RFC-0237)
                        → X2 data paths (#355 DONE @ RFC-0238)
                          → X2 apply-diff (#356 DONE @ RFC-0239)
                            → X3 M6 (#357 DONE @ RFC-0240; was first OPEN `#358`)
                              → X close RFC-0226 (#358 DONE)
                              → QUEUE X closed; Pack 2 DONE
                              → Phase Y `#359`–`#363` DONE; audit K `#376`+ перед `#364`–`#375`
```
...
```text
Q0 wiring (#275 DONE)
  → Q1 unique identity (#276 DONE @ RFC-0165)
    → Q1 model light observe (#277 DONE @ RFC-0166)
      → Q1 model verify context (#278 DONE @ RFC-0167)
        → Q1 reach observation independence (#279 DONE @ RFC-0168)
          → Q1 reach NAT (#280 DONE @ RFC-0169)
            → Q1 reach evidence (#281 DONE @ RFC-0170)
              → Q2 lifecycle revision (#282 DONE @ RFC-0171)
                → Q2 backend≠model (#283 DONE @ RFC-0172)
                  → Q2 invite atomicity (#284 DONE @ RFC-0173)
                    → Q close RFC-0164 (#285 DONE)
```
...
```text
P0 wiring (#266 DONE)
  → P1 freshness (#267 DONE @ RFC-0157)
    → P1 applied-runtime (#268 DONE @ RFC-0158)
      → P1 model triple (#269 DONE @ RFC-0159)
      → P1 reachability endpoint (#270 DONE @ RFC-0160)
          → P1 EVM honesty (#271)
            → P2 lifecycle (#272)
              → P2 Help routing (#273 DONE @ RFC-0163)
                → P close RFC-0156 (#274 DONE)
```
...
| Повний аудит `6007442` (2026-09-23): K1–K5 + D + acceptance | [`docs/audit-6007442-plan.md`](docs/audit-6007442-plan.md); K `#376`–`#382` **DONE**; Y `#359`–`#375` **DONE**; `#383`–`#389` **DONE** (closed); live tip → Phase Z `#391` |
| Desktop chrome density (post-audit A) | [`docs/phase-z-plan.md`](docs/phase-z-plan.md); `#390`–`#397`; `#390` **DONE**; **перший OPEN `#391`**; RFC-0244 reserved |
