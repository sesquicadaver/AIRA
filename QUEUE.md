...
**Handoff:** Phase X first OPEN `#346` (після `#345` select API DONE). Tip: `#342` DONE @ Analyze-379 / RFC-0215; `#343`–`#345` DONE @ Analyze-380…382 / RFC-0227…0228.
                  ...
                  → W1 reuse candidate (#340 DONE @ RFC-0224)
                    → W1 result/verify binding (#341 DONE @ RFC-0225)
                      → W close RFC-0215 (#342 DONE)
                        → Phase X Pack 2 (#343–#345 DONE; first OPEN `#346`)
```
...
**Статус фази:** **IN PROGRESS** @ RFC-0226 reserved (після **QUEUE W closed** @ RFC-0215). `#343`–`#345` DONE; **перший OPEN:** `#346`.
...
| 346 | **OPEN** | Analyze-383 — Request profile → snapshot (M3) | overrides/excludes; mid-flight safe; RFC-0229 | snapshot green | CLI (#347) |
| 347 | **OPEN** | Analyze-384 — CLI supported-contract parity (M4) | no no-op flags; RFC-0230 | CLI parity | Settings GUI (#348) |
...
**Перший OPEN:** `#346` (Request profile → snapshot). Tip: `#345` DONE @ Analyze-382 / RFC-0228.

```text
X0 wiring (#343 DONE)
  → X1 inventory M1 (#344 DONE)
    → X1 select API M2 (#345 DONE)
      → X1 profile→snapshot M3 (#346 OPEN / first OPEN)
        → X1 CLI parity M4 (#347)
          → X2 Settings Models (#348)
...
| Pack 2 local multi-model GUI (aira-current 2026-09-13 + M1–M6) | [`docs/phase-x-plan.md`](docs/phase-x-plan.md); `#343`–`#358` **IN PROGRESS**; first OPEN `#346`; RFC-0226 reserved |
