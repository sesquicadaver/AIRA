...
## Активна черга (лінійна) — Phase R: Actionable Desktop Connection UX (post-Q)

План: [`docs/phase-r-plan.md`](docs/phase-r-plan.md). Канон: [`docs/desktop-ux.md`](docs/desktop-ux.md). QUEUE Q closed @ RFC-0164.  
**Не** GPU marketplace; **не** LLM у `aira-core`; consolidating RFC-0174 у `#294` (file-free до close).
...
| 288 | **OPEN** | Analyze-324 — Promote connect controls | профіль + invite видимі без обовʼязкового collapse | desktop + тести + RFC-D | language (#289) |
| 289 | **OPEN** | Analyze-325 — Human-primary mesh/strip | людський conclusion primary; raw enum secondary | desktop + тести + RFC-D | footer (#290) |
| 290 | **OPEN** | Analyze-326 — Actionable problem/strip | user-facing next action; wire-id не єдиний copy | desktop + тести + RFC-D | Help (#291) |
| 291 | **OPEN** | Analyze-327 — Help connect scenario | invite→profile→restart у help uk/en | docs/help + тести + RFC-D | UK (#292) |
| 292 | **OPEN** | Analyze-328 — UK mesh/discovery parity | EN==UK gaps mesh/discovery | i18n + тести + RFC-D | cold-start (#293) |
...
**IN PROGRESS.** `#286`–`#287` **DONE**. First OPEN: **`#288`**. Consolidating RFC-0174 file-free until `#294`.

```text
R0 wiring (#286 DONE)
  → R1 Connection CTA (#287 DONE @ RFC-0175)
    → R2 promote connect controls (#288 OPEN)
      → R3 human-primary language (#289)
        → R4 problem/strip actions (#290)
          → R5 Help scenario (#291)
            → R6 UK parity (#292)
              → R7 cold-start (#293)
                → R close RFC-0174 (#294)
```

Анти-місія README **ніколи** в QUEUE.


**Наступне (історично Q):** Phase R активовано — див. секцію Phase R вище.
