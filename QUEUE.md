...
**Не** GPU marketplace; **не** LLM у `aira-core`; consolidating RFC-0192 у `#312`; **не** dial live-honesty (`#307`) у wiring.
...
| 307 | **OPEN** | Analyze-343 — Dial evidence ≠ live session | evidence ≠ live_session_count; UI last-check; fix Some(1) test | runtime + тести + RFC-D | dial async (#308) |
| 308 | **OPEN** | Analyze-344 — Opt-in dial off UI-thread | dial via jobs/slot; no UI block_on; F1/nav during attempt | desktop + тести + RFC-D | observe fail (#309) |
| 309 | **OPEN** | Analyze-345 — Observe miss fail durable | Pending/Ready/Failed; no rehash storm; admit unchanged | flow + тести + RFC-D | Quit∥Submit (#310) |
| 310 | **OPEN** | Analyze-346 — Quit∥Submit completion | deferred Stop→Close **або** reject без sticky flag | desktop + тести + RFC-D | AddressBook (#311) |
| 311 | **OPEN** | Analyze-347 — Dial AddressBook candidate honesty | failed dial не тихо overwrite book | runtime + тести + RFC-D | RFC close (#312) |
| 312 | **OPEN** | Analyze-348 — RFC-0192 + close | consolidating RFC-0192; QUEUE T closed | RFC-0192; QUEUE T closed | — |

### Наступний цикл (Phase T)

**IN PROGRESS.** `#306` **DONE**. First OPEN: **`#307`**. Consolidating RFC-0192 file-free until `#312`.

```text
T0 wiring (#306 DONE)
  → T1 dial evidence ≠ live (#307 OPEN)
    → T1 dial off-UI (#308)
      → T1 observe fail durable (#309)
        → T2 Quit∥Submit (#310)
          → T2 AddressBook candidate (#311)
            → T close RFC-0192 (#312)
```
