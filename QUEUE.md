...
| # | Status | Analyze | Атомарний scope | Done when | Не в цьому рядку |
|---|--------|---------|-----------------|-----------|------------------|
| 275 | **DONE** | ~~Analyze-310 — Phase Q wiring~~ | plan + QUEUE `#275`–`#285` + `phase_q_doc`; RFC-0164 reserved; fmt tip | plan IN PROGRESS; `#275`–`#283` DONE @ RFC-0172; first OPEN `#284` | identity (#276) |
...
| 284 | **OPEN** | Analyze-319 — Invite import atomicity | checks before TrustStore/book writes; commit/rollback | desktop + тести + RFC-D | RFC close (#285) |
| 285 | **OPEN** | Analyze-320 — RFC-0164 + close | consolidating RFC-0164; QUEUE Q closed | RFC-0164; QUEUE Q closed | — |

### Наступний цикл (Phase Q)

**IN PROGRESS.** `#275`–`#283` **DONE** @ RFC-0172. First OPEN: **`#284`**. Consolidating RFC-0164 file-free until `#285`.
