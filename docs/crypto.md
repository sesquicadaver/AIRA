...
**Primary signer** (Analyze-22 / QUEUE `#190`): `aira_csu::support::{local_identity, local_signature, local_signature_over}` use `active_identity` / `active_signature`. `active_signature` returns `Result` and **does not** fall back to `local_test_signature` (RFC-0088). Demo/test: default primary is `aira:identity:local-test` (explicit; `reset_primary_signer`). A registered node identity without a signing key fails closed (`NoSigningKey`). `LocalSession` does not swallow identity/trust/tenant load errors. When a node identity is registered, OperationalPlane + basic CSU emits carry that `key_ref`. The plane itself remains a C1 reference/demo ([operational-plane.md](operational-plane.md)), not a production event runtime.

**Thread-local crypto** (QUEUE `#196` / RFC-0094): `bind_thread_crypto(ring, primary)` installs a thread-scoped keyring + primary for embed/tests. `active_signature` / `verify_ed25519` / `register_keyring` use that bind while the guard lives; the process OnceLock is unchanged. Drop restores the previous thread bind (or process default). Isolation: `thread_crypto_scopes_do_not_leak`.
...
**Канон (`#387` shared-tip rule):** [`QUEUE.md`](../QUEUE.md) — **перший OPEN `#392`** ([`phase-z-plan.md`](phase-z-plan.md); `#390`–`#391` DONE). Audit `#376`–`#389` **DONE** (closed provenance). Phase C [`phase-c-plan.md`](phase-c-plan.md) і рядки нижче — закритий provenance / deferred notes, не competing tip.
