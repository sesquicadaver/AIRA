...
- Generate-local payload `constraints.network = none` is a separate AIRA-mediated adapter rule ([`local-node.md`](local-node.md) / RFC-0116): the generate adapter opens no sockets; the child is not an OS network-off sandbox. Opt-in Landlock FS (`AIRA_LLM_LANDLOCK`; `#225` / RFC-0118), opt-in seccomp (`AIRA_LLM_SECCOMP`; `#226` / RFC-0119), opt-in netns (`AIRA_LLM_NETNS`; `#227` / RFC-0120, offline argv; ollama + netns fail-closed), and opt-in sandbox-required (`AIRA_LLM_SANDBOX_REQUIRED`; `#228` / RFC-0121, missing kernel/OS fail-closed) are ProcessBackend child restricts, not that adapter contract.
...
| execution-llm | `text.generate.local` via bound backend + injected activate gate; MockBackend default on OperationalPlane; ProcessBackend opt-in (not CI); Desktop Work `POST /v1/problems` (`#209`–`#216` DONE @ RFC-0104); process-boundary hardening Phase L `#217`–`#223` **DONE** @ RFC-0111; OS child sandbox Phase M `#224` **DONE**; Landlock FS `#225` **DONE** @ RFC-0118; seccomp `#226` **DONE** @ RFC-0119; netns `#227` **DONE** @ RFC-0120; sandbox-required `#228` **DONE** @ RFC-0121; `#229`–`#230` OPEN |
