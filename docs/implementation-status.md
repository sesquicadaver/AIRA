...
Model layer (EVO-3): D0–D3 `#53`–`#60` **DONE** on main (schemas + inventory + compatibility + default-DENY gate). Addendum D4 `#61`–`#64` OPEN (ALLOW → local quarantine → verify → activate; no remote HTTP in this wave). Not Core. Plan: [phase-d-plan.md](phase-d-plan.md) §6a. Not a C1 pipeline step.
