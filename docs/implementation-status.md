...
**Status:** **Reference v0.2** (Analyze-181 / QUEUE `#151`; Phase G `#120`–`#151` **DONE** @ RFC-0069). Phase H Protocol depth → **v0.3** target ([`phase-h-plan.md`](phase-h-plan.md); `#152`–`#154` DONE; `#155` **OPEN**). Map of what this repository implements versus Book 0–IV, Schema Pack, Conformance, and the basic CSU set. This is **not** a new architecture and **does not** add code to fill gaps beyond the active QUEUE atom.

**Navigation:** [`docs/README.md`](README.md) · **Queue:** [`QUEUE.md`](../QUEUE.md) (Phase H `#155`+) · **Phase H plan:** [`phase-h-plan.md`](phase-h-plan.md) · **Phase G:** [`phase-g-plan.md`](phase-g-plan.md) · **RFC:** [`AIRA-RFC-0069`](../specs/rfc/AIRA-RFC-0069-phase-g-reference-v0.2.md)
...
| Durable event log | Book I / Book IV §6.3 | `LocalSession` → `.aira/events/event-log.json`; `aira_event::EventHashChain` (#154) | `corrupt_event_log_recovered_and_writable`; `event_log_hash_chain_tip_append_verify_and_mid_tamper_detect` | **PARTIAL** | JSON file + in-memory hash-chain tip (#154); session wire later (#157); corrupt still full reset (#142) until prefix (#155) |
