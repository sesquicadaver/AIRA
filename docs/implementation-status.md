...
| Operational pipeline PS → Interpret → Reduce → Materialize → Verify → VRA | Book 0 §4; B0-003; OP-001 (**legacy example**; RFC-0242) | `aira_flow::OperationalPlane` (C1 **reference/demo**) | `c1.pipeline.process_executor_executed`; VRA fixtures; historical `c1.pipeline.calculate_2_plus_2` / `calculate_two_plus_two_demo` (non-normative) | **DONE** | Normative C1 smoke is process `Executed` ≠ VERIFIED; OP-001 math is non-normative |
...
| C1 | Local operational node | `run_c1` | `c1.pipeline.process_executor_executed`, `c1.csu.manifests`, `c1.result.verified_completeness`, `c1.result.extended_fields`, `c1.failure.to_evidence` | **yes** | **DONE** (RFC-0242: no mandatory Calculate 2+2) |
