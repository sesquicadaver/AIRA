...
| LLM Execution CSU (local generate) | Book I §2; Phase K `#211` | `csu/execution-llm` | `mock_backend_completes_valid_generate_local`; `missing_backend_is_capsule_failed`; `wrong_action_is_capsule_failed`; `extra_properties_fail_closed` | **PARTIAL** | MockBackend only; not on OperationalPlane (`#213`); activate gate `#214`; RFC-0106 |
...
**Phase K `#209`–`#216` IN PROGRESS** (не змінює anti-mission): Local LLM Execution CSU; [`phase-k-plan.md`](phase-k-plan.md); `#209` wiring **DONE**; `#210` generate-local schema **DONE** (RFC-0105); `#211` `execution-llm` mock **DONE** (RFC-0106); first OPEN `#212`; consolidating RFC-0104 at `#216`.
