...
| LLM Execution CSU (local generate) | Book I §2; Phase K `#211` `#212` `#213` | `csu/execution-llm`; `csu/reduction-basic` catalog; `aira-flow` plane | `mock_backend_completes_valid_generate_local`; `non_math_prompt_completes_via_execution_llm_mock`; `calculate_two_plus_two_stays_execution_basic`; `c1.pipeline.calculate_2_plus_2` | **PARTIAL** | MockBackend on OperationalPlane (`#213` / RFC-0108); generate is CapsuleCompleted not VERIFIED; activate gate `#214`; RFC-0106 |
...
**Phase K `#209`–`#216` IN PROGRESS** (не змінює anti-mission): Local LLM Execution CSU; [`phase-k-plan.md`](phase-k-plan.md); `#209` wiring **DONE**; `#210` generate-local schema **DONE** (RFC-0105); `#211` `execution-llm` mock **DONE** (RFC-0106); `#212` Reduction bind **DONE** (RFC-0107); `#213` Plane register **DONE** (RFC-0108); first OPEN `#214`; consolidating RFC-0104 at `#216`.
