...
| Durable event log | Book I / Book IV §6.3 | `LocalSession` → `.aira/events/event-log.json`; `aira_event::EventHashChain` (#154) | `corrupt_event_log_recovered_and_writable`; `corrupt_trailing_event_log_recovers_valid_prefix`; `event_log_hash_chain_tip_append_verify_and_mid_tamper_detect` | **PARTIAL** | JSON + hash-chain tip (#154) + prefix recovery (#155); session wire (#157); durable backend (#156) |
