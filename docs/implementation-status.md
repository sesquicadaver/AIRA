...
| LLM Execution CSU (local generate) | Book I §2; Phase K `#211` `#212` `#213` `#214` `#215` | `csu/execution-llm`; `csu/reduction-basic` catalog; `aira-flow` plane | `inactive_model_is_capsule_failed`; `missing_process_binary_is_capsule_failed`; `mock_backend_completes_valid_generate_local`; `default_plane_keeps_mock_backend`; `generate_without_activate_is_capsule_failed`; `non_math_prompt_completes_via_execution_llm_mock`; `calculate_two_plus_two_stays_execution_basic`; `c1.pipeline.calculate_2_plus_2` | **PARTIAL** | Process backend (`#215` / RFC-0110) fail-closes on missing CLI; MockBackend remains plane/CI default; activate gate (`#214` / RFC-0109); generate is CapsuleCompleted not VERIFIED; Desktop `#216`; RFC-0106 |
...
**Phase K `#209`–`#216` IN PROGRESS** (не змінює anti-mission): Local LLM Execution CSU; [`phase-k-plan.md`](phase-k-plan.md); `#209` wiring **DONE**; `#210` generate-local schema **DONE** (RFC-0105); `#211` `execution-llm` mock **DONE** (RFC-0106); `#212` Reduction bind **DONE** (RFC-0107); `#213` Plane register **DONE** (RFC-0108); `#214` Activate gate **DONE** (RFC-0109); `#215` Process backend **DONE** (RFC-0110); first OPEN `#216`; consolidating RFC-0104 at `#216`.
