...
| Settlement / Audit protocol | Book II §15; B2-011 | receipt schema + `SettlementReceiptStore` JSONL (`aira:settlement:receipts-jsonl:v1`) + privacy gate (`#172`–`#174`); RFC-0080 | `settlement_receipt_schema_loads`; `settlement_receipt_store_*`; `b2_011_settlement_privacy_smoke`; `validate_settlement_privacy`; `SETTLEMENT_PRIVACY_FORBIDDEN_KEYS` | **ABSENT** | `run_c4` `#175`; status PARTIAL at `#176` |
