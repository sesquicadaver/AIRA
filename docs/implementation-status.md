...
| LLM Execution CSU (local generate) | Book I §2; Phase K `#211` `#212` | `csu/execution-llm`; `csu/reduction-basic` catalog | `mock_backend_completes_valid_generate_local`; `missing_backend_is_capsule_failed`; `wrong_action_is_capsule_failed`; `extra_properties_fail_closed`; `non_math_prompt_binds_generate_local`; `calculate_2_plus_2_binds_math_eval_safe` | **PARTIAL** | MockBackend only; Reduction selects generate-local (`#212` / RFC-0107); not on OperationalPlane (`#213`); activate gate `#214`; RFC-0106 |
...
**Phase K `#209`–`#216` IN PROGRESS** (не змінює anti-mission): Local LLM Execution CSU; [`phase-k-plan.md`](phase-k-plan.md); `#209` wiring **DONE**; `#210` generate-local schema **DONE** (RFC-0105); `#211` `execution-llm` mock **DONE** (RFC-0106); `#212` Reduction bind **DONE** (RFC-0107); first OPEN `#213`; consolidating RFC-0104 at `#216`.
