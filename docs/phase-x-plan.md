...
**Статус:** **IN PROGRESS** (активовано `#343` wiring; `#343`–`#347` **DONE**; first OPEN `#348`; RFC-0226 reserved file-free until `#358`).  
...
2. First OPEN репо = `#348` після `#347` CLI parity; лінійність `#N` після `#N-1`.
    ...
    → #345 X1 select API M2 DONE @ RFC-0228
      → #346 X1 profile→snapshot M3 DONE @ RFC-0229
        → #347 X1 CLI parity M4 DONE @ RFC-0230
          → #348 X2 Settings Models GUI (first OPEN)
...
QUEUE W closed @ RFC-0215
  ...
  → phase-x-plan **IN PROGRESS**; `#343`–`#347` DONE; first OPEN `#348`.
```
