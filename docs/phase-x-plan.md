...
**Статус:** **IN PROGRESS** (активовано `#343` wiring; `#343`–`#352` **DONE**; first OPEN `#353`; RFC-0226 reserved file-free until `#358`).  
...
2. First OPEN репо = `#353` після `#352` Human copy; лінійність `#N` після `#N-1`.
              ...
              → #350 X2 mock + result DONE @ RFC-0233
                → #351 X2 network/address + P UX DONE @ RFC-0234
                  → #352 X2 copy + IA DONE @ RFC-0235
                    → #353 X2 Help F1 model (first OPEN)
                      → #354 X2 Compare
...
QUEUE W closed @ RFC-0215
  ...
  → phase-x-plan **IN PROGRESS**; `#343`–`#352` DONE; first OPEN `#353`.
```
