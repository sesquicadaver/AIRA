...
**Статус:** **IN PROGRESS** (активовано `#343` wiring; `#343`–`#346` **DONE**; first OPEN `#347`; RFC-0226 reserved file-free until `#358`).  
...
2. First OPEN репо = `#347` після `#346` profile snapshot; лінійність `#N` після `#N-1`.
  ...
  → #344 X1 inventory M1 DONE @ RFC-0227
    → #345 X1 select API M2 DONE @ RFC-0228
      → #346 X1 profile→snapshot M3 DONE @ RFC-0229
        → #347 X1 CLI parity M4 (first OPEN)
...
QUEUE W closed @ RFC-0215
  ...
  → phase-x-plan **IN PROGRESS**; `#343`–`#346` DONE; first OPEN `#347`.
```
