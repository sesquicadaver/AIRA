...
NAT traversal; gossip; DHT; federation join; public HTTP bind; trust-delta over wire; mTLS.

## Relation to crypto docs

Peer **trust rotate** remains `identity trust rotate`. Node **secret rotate** remains `identity rotate` (Ed25519). Noise static is separate (`local.x25519`) and regenerated only if file missing (not rotated with Ed25519 in this slice).
