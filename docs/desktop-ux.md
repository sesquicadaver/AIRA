...
Кожна: висновок людською мовою, час спостереження, максимум одна коригувальна дія, технічні подробиці. Phase S `#300` **DONE** @ RFC-0187: Technical details opt-in `run_opt_in_peer_dial` + dial evidence. Phase S `#299` **DONE** @ RFC-0186: Connection always shows `conn_boundary_guidance` (setup ≠ remote; loopback ≠ dial). Phase R `#287` **DONE** @ RFC-0175: Connection primary CTA = `connection_cta::primary_connection_cta` (не лише Refresh). Phase R `#290` **DONE** @ RFC-0178: problem/strip next-step = `problem_action` (wire `help:`/`try:` secondary).
