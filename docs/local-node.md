...
- **Installed-product LLM** is opt-in (`scripts/installed-product-llm-acceptance.sh`, `#[ignore]` test). Fixture M6 (`phase_x_m6_*`) is not that gate. Default CI stays process-smoke C1 (`c1.pipeline.process_executor_executed`: Executed ≠ VERIFIED) plus VRA schema fixtures — not a runtime VRA pipeline and not `ollama run`.
