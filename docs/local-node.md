...
- **Installed-product LLM** is opt-in (`scripts/installed-product-llm-acceptance.sh`, `#[ignore]` test). Fixture M6 (`phase_x_m6_*`) is not that gate. Default CI stays process-smoke C1 (`c1.pipeline.process_executor_executed`: Executed ≠ VERIFIED) plus VRA schema fixtures — three separate statuses from legacy OP-001 math (`#384`); not a runtime VRA pipeline and not `ollama run`. GUI fix order: [`gui-audit-fix-plan.md`](gui-audit-fix-plan.md). A running node’s Work readiness follows the applied executor, not settings that were only saved.
- **Desktop:** Work uses one model selector (`Default: name`) with search, Compatible / Show all, and a shown N of M counter; Compare opens a second selector (B empty on enter; exit restores the prior solo pick). Five shared fitness labels (Ready / Prepare and run / Checking… / Needs action / Incompatible) appear on catalog rows; F1 Help documents **Prepare and run** and does not treat a `:cloud` name suffix as proof of locality. A **Local file** row may be prepared and visible in the catalog; on the host Ollama ProcessBackend that is **not** Work ready for generate (`#383` / RFC-0240 §8b / R3) — no file-executor is implied. Run, the shortcut, and submit share one availability check; the status strip says ready only when that readiness is true. Stop node is not a cancel. F1 docks beside the screen only from 880 px (900 and 1600 keep a central column of at least 560); at the default 560 px width Help floats so it does not squeeze Work. Settings order is Models, General, Connection; timeout (seconds, zero rejected) and operator network live under Advanced. The HTTP read waits 15s past the process deadline so a transport error is not reported as malformed HTTP. Saved and Applied are not repeated when they match. Per-request Work Required/Compare uses activate `host_cli_model` from **signed evidence** so `ollama run <model>` follows admission without node restart (RFC-0243; R2). Ollama ProcessBackend refuses local file-weight bindings (R3). `aira desktop start` / GUI Start sets `AIRA_LLM_BACKEND` / `AIRA_LLM_PROCESS_*` (+ optional `AIRA_LLM_PROCESS_TIMEOUT_MS` from Settings timeout UI, recorded on pidfile) on the spawned `aira-node`. Catalog Add/Prepare/Select/Verify and Work submit are mutually exclusive; Compare shows leg A as soon as it finishes. Default remains mock. Executed ≠ VERIFIED.
