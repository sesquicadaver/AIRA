...
- **Desktop:** Settings → Models may persist `llm_backend=process` + `llm_ollama_model` (RFC-0241). `aira desktop start` / GUI Start then sets `AIRA_LLM_BACKEND` / `AIRA_LLM_PROCESS_*` on the spawned `aira-node`. Default remains mock. Executed ≠ VERIFIED.
