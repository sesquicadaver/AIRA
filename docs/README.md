# AIRA documentation index

**Оновлено:** 2026-09-13. Канон виконання: [`QUEUE.md`](../QUEUE.md) (Phase W [`phase-w-plan.md`](phase-w-plan.md) `#331`–`#341` **DONE** / first OPEN `#342` (Pack 1 residual; RFC-0216…RFC-0225; RFC-0215 reserved); Phase X [`phase-x-plan.md`](phase-x-plan.md) `#343`–`#358` **QUEUED** (Pack 2 GUI; RFC-0226 reserved); Phase V [`phase-v-plan.md`](phase-v-plan.md) `#323`–`#330` **DONE** @ RFC-0208 (**QUEUE V closed**); Phase U [`phase-u-plan.md`](phase-u-plan.md) `#313`–`#322` **DONE** @ RFC-0198 (**QUEUE U closed**); Phase T [`phase-t-plan.md`](phase-t-plan.md) `#306`–`#312` **DONE** @ RFC-0192 (**QUEUE T closed**); Phase S [`phase-s-plan.md`](phase-s-plan.md) `#295`–`#305` **DONE** @ RFC-0182; **QUEUE S closed**; Phase Q `#275`–`#285` **DONE** @ RFC-0164; **QUEUE Q closed**; Phase R `#286`–`#294` **DONE** @ RFC-0174; **QUEUE R closed**; Phase P `#266`–`#274` **DONE** @ RFC-0156; QUEUE P closed; Phase O `#255`–`#265` **DONE** @ RFC-0146; QUEUE O closed; N-fix `#248`–`#254` **DONE** @ RFC-0139; QUEUE N-fix closed; Phase N `#231`–`#247` **DONE**; QUEUE N closed; Phase M `#224`–`#230` **DONE** @ RFC-0117; QUEUE M closed; Phase L `#217`–`#223` **DONE** @ RFC-0111; QUEUE L closed; Phase K `#209`–`#216` **DONE** @ RFC-0104; QUEUE K closed; Phase J `#199`–`#208` **DONE** @ RFC-0096; QUEUE J closed; Phase I `#184`–`#198` **DONE** @ RFC-0078; QUEUE I closed; Phase H `#152`–`#183` **DONE** @ RFC-0077; Phase G `#120`–`#151` **DONE**). Статус реалізації: [`implementation-status.md`](implementation-status.md) (**Reference v0.3-strict**; [`phase-w-plan.md`](phase-w-plan.md) IN PROGRESS; [`phase-x-plan.md`](phase-x-plan.md) QUEUED; [`phase-v-plan.md`](phase-v-plan.md) DONE @ RFC-0208; QUEUE V closed; [`phase-u-plan.md`](phase-u-plan.md) DONE @ RFC-0198; QUEUE U closed; [`phase-s-plan.md`](phase-s-plan.md) DONE @ RFC-0182; QUEUE S closed; [`phase-r-plan.md`](phase-r-plan.md); [`phase-q-plan.md`](phase-q-plan.md); [`phase-o-plan.md`](phase-o-plan.md); [`phase-n-fix-plan.md`](phase-n-fix-plan.md); [`phase-n-plan.md`](phase-n-plan.md); [`phase-m-plan.md`](phase-m-plan.md); [`phase-l-plan.md`](phase-l-plan.md); [`AIRA-RFC-0117`](../specs/rfc/AIRA-RFC-0117-phase-m-os-child-sandbox.md); [`AIRA-RFC-0111`](../specs/rfc/AIRA-RFC-0111-phase-l-execution-boundary.md); [`AIRA-RFC-0104`](../specs/rfc/AIRA-RFC-0104-phase-k-local-llm-csu.md); [`AIRA-RFC-0096`](../specs/rfc/AIRA-RFC-0096-phase-j-book-gap-local.md); [`AIRA-RFC-0078`](../specs/rfc/AIRA-RFC-0078-phase-i-semantic-stabilization.md); [`AIRA-RFC-0077`](../specs/rfc/AIRA-RFC-0077-phase-h-protocol-depth-v0.3.md)).

## Anti-mission (ніколи в product / QUEUE)

README §«What AIRA is not» — **не** backlog:

```text
GPU marketplace · LLM runtime · blockchain network · scheduler
Core що «вирішує задачі» замість CSU contracts
```

Деталі: [`phase-g-plan.md`](phase-g-plan.md) §3; [`canonical-terminology.md`](canonical-terminology.md) (out of Kernel ≠ out of project); [`product-requirements-trace.md`](product-requirements-trace.md).

## Phase plans (лінійна черга)

| Phase | Plan | QUEUE | Статус |
|-------|------|-------|--------|
| C | [`phase-c-plan.md`](phase-c-plan.md) | `#38`–`#52` | **DONE** |
| D | [`phase-d-plan.md`](phase-d-plan.md) | `#53`–`#74` | **DONE** |
| E | [`phase-e-plan.md`](phase-e-plan.md) + [`desktop-ux.md`](desktop-ux.md) | `#75`–`#106` | **DONE** |
| F | [`phase-f-plan.md`](phase-f-plan.md) | `#107`–`#119` | **DONE** |
| G | [`phase-g-plan.md`](phase-g-plan.md) | `#120`–`#151` | **DONE** (RFC-0069) |
| H | [`phase-h-plan.md`](phase-h-plan.md) | `#152`–`#183` | **DONE** (RFC-0077) |
| I | [`phase-i-plan.md`](phase-i-plan.md) | `#184`–`#198` | **DONE** (`#184` `#185` `#186` `#187` `#188` `#189` `#190` `#191` `#192` `#193` `#194` `#195` `#196` `#197` `#198` @ RFC-0078) |
| J | [`phase-j-plan.md`](phase-j-plan.md) | `#199`–`#208` | **DONE** (RFC-0096) |
| K | [`phase-k-plan.md`](phase-k-plan.md) | `#209`–`#216` | **DONE** (RFC-0104); QUEUE K closed |
| L | [`phase-l-plan.md`](phase-l-plan.md) | `#217`–`#223` | **DONE** @ RFC-0111; QUEUE L closed |
| M | [`phase-m-plan.md`](phase-m-plan.md) | `#224`–`#230` | **DONE** @ RFC-0117; QUEUE M closed |
| N | [`phase-n-plan.md`](phase-n-plan.md) | `#231`–`#247` | **DONE** @ RFC-0123; QUEUE N closed |
| N-fix | [`phase-n-fix-plan.md`](phase-n-fix-plan.md) | `#248`–`#254` | **DONE** @ RFC-0139 (QUEUE N-fix closed; global live PARTIAL) |
| O | [`phase-o-plan.md`](phase-o-plan.md) + [`desktop-ux.md`](desktop-ux.md) | `#255`–`#265` | **DONE** @ RFC-0146 (QUEUE O closed) |
| P | [`phase-p-plan.md`](phase-p-plan.md) + [`desktop-ux.md`](desktop-ux.md) | `#266`–`#274` | **DONE** @ RFC-0156; QUEUE P closed |
| Q | [`phase-q-plan.md`](phase-q-plan.md) + [`desktop-ux.md`](desktop-ux.md) | `#275`–`#285` | **DONE** @ RFC-0164; **QUEUE Q closed** |
| R | [`phase-r-plan.md`](phase-r-plan.md) + [`desktop-ux.md`](desktop-ux.md) | `#286`–`#294` | **DONE** @ RFC-0174; QUEUE R closed |
| S | [`phase-s-plan.md`](phase-s-plan.md) + [`desktop-ux.md`](desktop-ux.md) | `#295`–`#305` | **DONE** @ RFC-0182; **QUEUE S closed** |
| T | [`phase-t-plan.md`](phase-t-plan.md) + [`desktop-ux.md`](desktop-ux.md) | `#306`–`#312` | **DONE** @ RFC-0192 (**QUEUE T closed**; no OPEN T atoms) |
| U | [`phase-u-plan.md`](phase-u-plan.md) + [`desktop-ux.md`](desktop-ux.md) | `#313`–`#322` | **DONE** @ RFC-0198 (**QUEUE U closed**) |
| V | [`phase-v-plan.md`](phase-v-plan.md) + [`repair-package-1.md`](repair-package-1.md) | `#323`–`#330` | **DONE** @ RFC-0208 (`#323`–`#330`; QUEUE V closed) |
| W | [`phase-w-plan.md`](phase-w-plan.md) + [`repair-package-1.md`](repair-package-1.md) | `#331`–`#342` | **IN PROGRESS** (`#331`–`#341` DONE @ RFC-0225; first OPEN `#342`; RFC-0215 reserved; handoff Pack 2 → Phase X) |
| X | [`phase-x-plan.md`](phase-x-plan.md) | `#343`–`#358` | **QUEUED** (Pack 2 multi-model GUI; після W; RFC-0226 reserved; `aira-current.md` 2026-09-13) |

## Core reference

| Doc | Topic |
|-----|-------|
| [`implementation-status.md`](implementation-status.md) | Book 0–IV matrix; C0–C5; Phase gates; **Reference v0.3-strict**; QUEUE J closed; QUEUE K closed @ RFC-0104; QUEUE L closed @ RFC-0111; QUEUE M closed @ RFC-0117; QUEUE N closed @ RFC-0123; N-fix `#248`–`#254` DONE; QUEUE N-fix closed; Phase O `#255`–`#265` DONE @ RFC-0146; QUEUE O closed; Phase P `#266`–`#274` DONE @ RFC-0156; QUEUE P closed; Phase Q `#275`–`#285` DONE @ RFC-0164; QUEUE Q closed; Phase R `#286`–`#294` DONE @ RFC-0174; QUEUE R closed; Phase W [`phase-w-plan.md`](phase-w-plan.md) **IN PROGRESS** (`#331`–`#341` DONE @ RFC-0225; first OPEN `#342`; RFC-0215 reserved; `phase_w_doc.rs`); Phase V [`phase-v-plan.md`](phase-v-plan.md) **DONE** @ RFC-0208 (`#323`–`#330`; QUEUE V closed; `phase_v_doc.rs`); Phase U [`phase-u-plan.md`](phase-u-plan.md) **DONE** @ RFC-0198 (**QUEUE U closed**); Phase T [`phase-t-plan.md`](phase-t-plan.md) **DONE** @ RFC-0192 (**QUEUE T closed**); Phase S [`phase-s-plan.md`](phase-s-plan.md) **DONE** @ RFC-0182; **QUEUE S closed** ( `phase_s_doc.rs` / `phase_r_doc.rs` / `phase_q_doc.rs`) |
| [`rfc-p-promotion.md`](rfc-p-promotion.md) | RFC-P: Book V → operational лише через promotion (`#177`) |
| [`operational-plane.md`](operational-plane.md) | C1 reference/demo plane (не production runtime) |
| [`conformance.md`](conformance.md) | C0–C5 runners; v0.3 index; security baseline |
| [`ci-governance.md`](ci-governance.md) | GitHub Actions merge gate |
| [`demo.md`](demo.md) | Local 2+2 and failure paths |
| [`canonical-terminology.md`](canonical-terminology.md) | Forbidden Core vocabulary; out of Kernel ≠ out of project |
| [`product-requirements-trace.md`](product-requirements-trace.md) | Product MUST trace (Repair Pack 0); schema/CLI ≠ product DONE |
| [`repair-package-0.md`](repair-package-0.md) | Pack 0 close @ RFC-0207 |
| [`repair-package-1.md`](repair-package-1.md) | Pack 1 structural DONE @ RFC-0208; residual honesty Phase W IN PROGRESS |

## CSU & models

| Doc | Topic |
|-----|-------|
| [`csu-development.md`](csu-development.md) | Manifest, trait, basic set |
| [`model-acquisition-policy.md`](model-acquisition-policy.md) | Download/publish fail-closed |
| [`csu-sdk.md`](csu-sdk.md) | Third-party CSU contract (`#145`; not marketplace) |

## Local node & network (post-MVP)

| Doc | Topic |
|-----|-------|
| [`local-node.md`](local-node.md) | `.aira` layout, HTTP, identity; generate-local `network=none` honesty (RFC-0116) |
| [`crypto.md`](crypto.md) | Ed25519, Noise, TLS, keyring |
| [`peer-link.md`](peer-link.md) | Peer listen, trust, relay, DHT prototypes |
| [`runbook-systemd.md`](runbook-systemd.md) | Long-running daemons |

## Desktop (Developer Preview)

| Doc | Topic |
|-----|-------|
| [`desktop-ux.md`](desktop-ux.md) | Product UX canon (Phase O: Work / System / Settings + F1) |
| [`desktop-network-profiles.md`](desktop-network-profiles.md) | P0–P6 fail-closed rules |
| [`desktop-gui.md`](desktop-gui.md) | `aira-desktop` GUI |
| [`desktop-launcher.md`](desktop-launcher.md) | Linux `.desktop` launcher |
| [`desktop-peer.md`](desktop-peer.md) | Supervised peer lifecycle |
| [`desktop-invite.md`](desktop-invite.md) | PeerInvite file + QR PNG |
| [`desktop-federation.md`](desktop-federation.md) | P5 federation GUI |
| [`desktop-discovery.md`](desktop-discovery.md) | P6 STUN/discv (Dev) |
| [`desktop-packaging.md`](desktop-packaging.md) | Linux tarball + `.deb` pipeline (`#150`) |
| [`desktop-packaging-macos.md`](desktop-packaging-macos.md) | macOS `.app` tarball + codesign/notarize helper (`#148`) |
| [`desktop-packaging-windows.md`](desktop-packaging-windows.md) | Windows zip + MSI pipeline (`#149`) |

## Releases & security

| Doc | Topic |
|-----|-------|
| [`RELEASE_NOTES-v0.1.0-alpha.1.md`](RELEASE_NOTES-v0.1.0-alpha.1.md) | Alpha release notes |
| [`../SECURITY.md`](../SECURITY.md) | Reporting, controls, out-of-scope |

## Historical / provenance

| Doc | Topic |
|-----|-------|
| [`../NEXT_PROBLEM.md`](../NEXT_PROBLEM.md) | Desktop one-click problem (RESOLVED → Phase E) |
| [`../EVO-1.md`](../EVO-1.md) | Repo analysis snapshot |
| [`../EVO-2.md`](../EVO-2.md) | Evolution recommendations |
