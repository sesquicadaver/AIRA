...
A confirmed remote dial path is **opt-in** under **Technical details → Peer dial**: trusted peer identity + **explicit** dial address. The dial attempt runs **off the UI thread** so Help (F1) and navigation stay available while it is in progress. Success stores handshake evidence and may show a **last confirmed handshake** (session closed after the check) — it does **not** invent a live session or DIRECT reachability from setup alone, and Desktop listen stays loopback by default.
