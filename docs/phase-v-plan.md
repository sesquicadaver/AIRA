...
**Статус:** **DONE** @ [AIRA-RFC-0208](../specs/rfc/AIRA-RFC-0208-phase-v-admission-integrity.md) (активовано 2026-09-10; `#323`–`#330` **DONE**; **QUEUE V closed**; no OPEN V atoms).  
...
**Closure:** consolidating **RFC-0208** **DONE** @ `#330` (id was confirmed free 2026-09-10 until close).  
...
Pack 0 зафіксував продуктове право на вибір моделі/ресурсів. Phase U замкнула cross-path honesty результатів. Але **вибір користувача ще не проходив незмінним знімком** від UI/CLI/HTTP до reuse і виконавця:

1. Submit приймав лише `text` — немає immutable admission snapshot (модель/набір, content identity, gen params, placement, privacy/budget, fallback, require-new vs allow-reuse).
2. Reuse-index ключувався лише hash тексту → результат іншої моделі міг «задовольнити» вимогу виконати моделлю X.
3. `activate_verified()` хешував після копіювання **без** звіряння з `VerifiedPointer.content_hash`.
4. Problem→Capsule→Output→Result / executor facts для generate-local не були замкнуті як обов’язковий binding path.

```text
QUEUE U closed (#313–#322 / RFC-0198)
  + Pack 0 DONE (RFC-0207)
  → Phase V DONE: admission integrity (Repair Pack 1) @ RFC-0208
...
|----|---------|------|-----------|------------------|
| `#323` | V0 | Phase V wiring | план + QUEUE + `phase_v_doc`; RFC-0208 reserved | snapshot type (#324) |
| `#324` | V1 | Admission snapshot type + persist | **DONE** @ RFC-0209 | Submit API surface (#325) |
| `#325` | V1 | Submit API carries constraints | **DONE** @ RFC-0210 | reuse key (#326) |
| `#326` | V1 | Reuse after constraints | **DONE** @ RFC-0211 | activate hash (#327) |
| `#327` | V1 | `activate_verified` hash continuity | **DONE** @ RFC-0212 | capsule binding (#328) |
| `#328` | V1 | Capsule↔Output↔Result binding | **DONE** @ RFC-0213 | e2e suite (#329) |
| `#329` | V2 | E2E fail-closed tests | **DONE** @ RFC-0214 | Pack 2 GUI |
| `#330` | V | RFC-0208 + close | **DONE** @ RFC-0208; QUEUE V closed; `repair-package-1.md` DONE | Pack 2 |
      ...
      → #326 V1 reuse after constraints DONE @ RFC-0211
        → #327 V1 activate hash continuity DONE @ RFC-0212
          → #328 V1 capsule/result binding DONE @ RFC-0213
            → #329 V2 e2e fail-closed DONE @ RFC-0214
              → #330 V close RFC-0208 (DONE; QUEUE V closed)
```
...
- Consolidating (`#330`): **AIRA-RFC-0208** — **DONE** (id confirmed free 2026-09-10 until close).
- Per-atom RFC-D: `#324`→**0209** … `#329`→**0214**; `#330`→0208 consolidating.
...
QUEUE U closed @ RFC-0198
  ...
  → wiring `#323` DONE; `#324`–`#329` DONE @ RFC-0209…0214; `#330` DONE @ RFC-0208; QUEUE V closed.
```
