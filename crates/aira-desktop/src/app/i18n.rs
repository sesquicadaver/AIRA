...
static EN: Labels = Labels {
    ...
    work_executor_hint: "Auto uses the Settings tip when available. Specific requires one available catalog model. Compare runs two distinct models sequentially — no silent substitute. Choice ≠ VERIFIED. Math does not need a model (and cannot use Compare).",
    ...
    work_capability_math: "Capability: calculation (no model required)",
    work_capability_generate: "Capability: local text generation",
    work_mock_banner: "Demo executor (reference mock) — text generation does not run a real local model. Choice ≠ VERIFIED.",
...
static UK: Labels = Labels {
    ...
    work_executor_hint: "Авто бере tip з Параметри→Моделі, якщо доступний. Конкретна — одна доступна модель з каталогу. Порівняти — дві різні моделі послідовно, без тихої підміни. Вибір ≠ VERIFIED. Розрахунок не потребує моделі (і не для Порівняти).",
    ...
    work_capability_math: "Можливість: розрахунок (модель не потрібна)",
    work_capability_generate: "Можливість: локальна генерація тексту",
    work_mock_banner: "Демо-виконавець (reference mock) — генерація тексту не запускає реальну локальну модель. Вибір ≠ VERIFIED.",
