...
static EN: Labels = Labels {
    ...
    sys_model_triple_hint: "Selected, ready, used-in-result, and submit executor are independent — missing a model does not block built-in calculation; ready ≠ process LLM.",
    ...
    work_cancel_honesty: "Esc does not cancel. Stop node is best-effort and may leave an orphan process — no silent cancel via closing HTTP.",
    work_executor: "Executor:",
    work_executor_auto: "Auto",
    work_executor_specific: "Specific model",
    work_executor_compare: "Compare",
    work_executor_hint: "Auto uses the Settings tip when available. Specific requires one available catalog model. Compare runs two distinct models sequentially — no silent substitute. Choice ≠ VERIFIED. Work needs a host process LLM (Settings → Models); reference mock is not product Work.",
    ...
    work_user_note: "Work requires at least one host LLM (Settings → Models process bind). Text generation never pretends a result was verified. Built-in math demo is legacy, not product acceptance.",
    ...
    settings_models_catalog_hint: "Local catalog: Scan / Add / Select / Prepare. Choice ≠ VERIFIED. selected ≠ ready ≠ used stays honest.",
    ...
    settings_ollama_hint: "Bind host Ollama for text.generate.local. Lists `ollama list` on this machine. Process ≠ mock. Executed ≠ VERIFIED. Activate tip is separate. Restart the node to apply.",
    ...
    settings_ollama_timeout: "Process timeout (ms):",
    settings_ollama_timeout_apply: "Apply timeout",
    settings_ollama_timeout_hint: "Optional generate deadline for process executor. Empty = backend default. Restart node to apply. Timed-out runs are not VERIFIED.",
...
static UK: Labels = Labels {
    ...
    sys_model_triple_hint: "Вибрана, готова, використана в результаті й виконавець submit — незалежні факти. Відсутність моделі не блокує вбудовані обчислення; готово ≠ process LLM.",
    ...
    work_cancel_honesty: "Esc не скасовує. Stop вузла — best-effort і може лишити orphan-процес; немає тихого cancel через закриття HTTP.",
    work_executor: "Виконавець:",
    work_executor_auto: "Авто",
    work_executor_specific: "Конкретна модель",
    work_executor_compare: "Порівняти",
    work_executor_hint: "Авто бере tip з Параметри→Моделі, якщо доступний. Конкретна — одна доступна модель з каталогу. Порівняти — дві різні моделі послідовно, без тихої підміни. Вибір ≠ VERIFIED. Work потребує host process LLM; reference mock не є продуктовим Work.",
    ...
    work_user_note: "Work вимагає щонайменше один host LLM (Параметри → Моделі, process). Генерація тексту ніколи не вдає VERIFIED. Вбудоване math-демо — legacy, не продуктове acceptance.",
    ...
    settings_models_catalog_hint: "Локальний каталог: Скан / Додати / Вибрати / Підготувати. Вибір ≠ VERIFIED. selected ≠ ready ≠ used лишається чесною.",
    ...
    settings_models_available: "available",
    settings_ollama_heading: "Ollama (process executor)",
    settings_ollama_hint: "Прив’язати хостовий Ollama для text.generate.local. Список — `ollama list` на цій машині. Process ≠ mock. Executed ≠ VERIFIED. Activate tip окремо. Перезапустіть node, щоб застосувати.",
    ...
    settings_ollama_timeout: "Таймаут процесу (мс):",
    settings_ollama_timeout_apply: "Застосувати таймаут",
    settings_ollama_timeout_hint: "Опційний дедлайн generate для process executor. Порожньо = типовий backend. Перезапустіть node. Таймаут ≠ VERIFIED.",
