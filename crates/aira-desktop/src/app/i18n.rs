...
static EN: Labels = Labels {
    ...
    settings_ollama_hint: "Bind host Ollama for text.generate.local. Lists `ollama list` on this machine. Process ≠ mock. Executed ≠ VERIFIED. Phase D activate remains separate. Restart the node to apply.",
    ...
    settings_ollama_bound: "Ollama bind + Phase D tip saved — restart node to apply process executor (executed ≠ VERIFIED).",
...
static UK: Labels = Labels {
    ...
    settings_models_available: "available",
    settings_ollama_heading: "Ollama (process executor)",
    settings_ollama_hint: "Прив’язати хостовий Ollama для text.generate.local. Список — `ollama list` на цій машині. Process ≠ mock. Executed ≠ VERIFIED. Phase D activate лишається окремо. Перезапустіть node, щоб застосувати.",
    ...
    settings_ollama_bound: "Прив’язку Ollama + Phase D tip збережено — перезапустіть node для process executor (executed ≠ VERIFIED).",
