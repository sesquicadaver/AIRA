...
fn phase_w_queue_all_done() {
    ...
    assert!(
        text.contains("| 343 | **DONE**")
            ...
            && text.contains("| 346 | **DONE**")
            && text.contains("| 347 | **DONE**")
            && text.contains("| 348 | **DONE**")
            && text.contains("| 349 | **DONE**")
            && text.contains("| 350 | **DONE**")
            && text.contains("| 351 | **DONE**"),
        "QUEUE #343–#351 must be DONE after Network/address honesty"
    );
    assert!(
        !text.contains("**Перший OPEN:** `#351`") && text.contains("**Перший OPEN:** `#352`"),
        "QUEUE tip must advance from #351 to #352 after Network/address honesty"
    );
    for n in 352..=358 {
        assert!(
            text.contains(&format!("| {n} | **OPEN**")),
            "QUEUE #{n} must be OPEN (Phase X after #351)"
        );
    }
}
...
#[test]
fn phase_w_rfc_0225_present() {
    let path = repo_root().join("specs/rfc/AIRA-RFC-0225-result-verify-task-binding.md");
    let text = std::fs::read_to_string(&path).expect("RFC-0225 missing");
    for needle in [
        "#341",
        "get_result",
        "swap",
        ...
        assert!(text.contains(needle), "RFC-0225 missing: {needle}");
    }
}

#[test]
fn phase_w_result_verify_binding_runtime_present() {
    let local = std::fs::read_to_string(repo_root().join("crates/aira-flow/src/local.rs")).unwrap();
    assert!(local.contains("#341") || local.contains("RFC-0225"));
    assert!(local.contains("artifact_binds_problem_lookup"));
    let reuse = std::fs::read_to_string(repo_root().join("crates/aira-flow/src/reuse.rs")).unwrap();
    assert!(reuse.contains("artifact_binds_problem_lookup"));
    let ver =
        std::fs::read_to_string(repo_root().join("csu/verification-basic/src/lib.rs")).unwrap();
    assert!(ver.contains("capsule_matches_event_problem"));
    assert!(ver.contains("generate_local_capsule_without_expression_is_not_verification_failed"));
    let lib = std::fs::read_to_string(repo_root().join("crates/aira-flow/src/lib.rs")).unwrap();
    assert!(lib.contains("get_result_by_problem_rejects_swapped_verified_artifact_ref"));
}

#[test]
fn phase_w_rfc_0224_present() {
    let path = repo_root().join("specs/rfc/AIRA-RFC-0224-reuse-candidate-independent-check.md");
    let text = std::fs::read_to_string(&path).expect("RFC-0224 missing");
    for needle in [
        "#340",
        "admit_reuse_candidate",
        "foreign",
        "fail-closed",
        "RFC-0215",
    ] {
        assert!(text.contains(needle), "RFC-0224 missing: {needle}");
    }
}

#[test]
fn phase_w_reuse_candidate_runtime_present() {
    let reuse = std::fs::read_to_string(repo_root().join("crates/aira-flow/src/reuse.rs")).unwrap();
    assert!(reuse.contains("admit_reuse_candidate"));
    assert!(reuse.contains("#340") || reuse.contains("RFC-0224"));
    assert!(reuse.contains("admit_rejects_foreign_math_result"));
    let plane = std::fs::read_to_string(repo_root().join("crates/aira-flow/src/plane.rs")).unwrap();
    assert!(plane.contains("admit_reuse_candidate"));
    let lib = std::fs::read_to_string(repo_root().join("crates/aira-flow/src/lib.rs")).unwrap();
    assert!(lib.contains("foreign_vra_under_correct_reuse_key_does_not_complete_via_reuse"));
}
...
#[test]
fn phase_w_rfc_0222_present() {
    let path = repo_root().join("specs/rfc/AIRA-RFC-0222-safe-weights-materialization.md");
    let text = std::fs::read_to_string(&path).expect("RFC-0222 missing");
    for needle in [
        "#338",
        "no-follow",
        "post-copy",
        ...
        assert!(text.contains(needle), "RFC-0222 missing: {needle}");
    }
}
...
fn phase_w_safe_materialize_runtime_present() {
    ...
    assert!(mat.contains("WEIGHTS_IO_BUF"));
    let err =
        std::fs::read_to_string(repo_root().join("csu/model-acquisition/src/error.rs")).unwrap();
    assert!(err.contains("SymlinkRejected"));
    assert!(err.contains("MaterializeHashMismatch"));
    let activate =
        std::fs::read_to_string(repo_root().join("csu/model-acquisition/src/activate.rs")).unwrap();
    assert!(activate.contains("materialize_weights_nofollow"));
    let verify =
        std::fs::read_to_string(repo_root().join("csu/model-acquisition/src/verify.rs")).unwrap();
    assert!(verify.contains("materialize_weights_nofollow"));
    let lib =
        std::fs::read_to_string(repo_root().join("csu/model-acquisition/src/lib.rs")).unwrap();
    assert!(lib.contains("activate_rejects_cache_dest_symlink_escape"));
}

#[test]
fn phase_w_rfc_0221_present() {
    let path = repo_root().join("specs/rfc/AIRA-RFC-0221-production-activation-trust.md");
    let text = std::fs::read_to_string(&path).expect("RFC-0221 missing");
    for needle in [
        "#337",
        "local-test",
        "ActivationTrustMode",
        "activation.trust.fixture",
        "RFC-0215",
    ] {
        assert!(text.contains(needle), "RFC-0221 missing: {needle}");
    }
}

#[test]
fn phase_w_production_activation_trust_runtime_present() {
    let gate =
        std::fs::read_to_string(repo_root().join("crates/aira-flow/src/activate_gate.rs")).unwrap();
    assert!(gate.contains("ActivationTrustMode"));
    assert!(gate.contains("ACTIVATION_TRUST_FIXTURE_REL"));
    assert!(gate.contains("#337") || gate.contains("RFC-0221"));
    assert!(gate.contains("without_local_test") || gate.contains("Production"));
    let activate =
        std::fs::read_to_string(repo_root().join("csu/model-acquisition/src/activate.rs")).unwrap();
    assert!(activate.contains("ActivateProductionTrust") || activate.contains("production"));
    let err =
        std::fs::read_to_string(repo_root().join("csu/model-acquisition/src/error.rs")).unwrap();
