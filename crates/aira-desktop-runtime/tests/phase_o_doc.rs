...
#[test]
fn phase_o_queue_262_done_263_open() {
    ...
    assert!(
        text.contains("| 263 | **OPEN**"),
        "QUEUE #263 must be first OPEN"
    );
    for n in 264..=265 {
        assert!(
            text.contains(&format!("| {n} | **OPEN**")),
...
fn phase_o_readme_and_docs_index() {
    ...
    assert!(readme.contains("#263") || readme.contains("first OPEN"));
...
fn phase_o_next_problem() {
    ...
    assert!(text.contains("#263") || text.contains("перший OPEN"));
    assert!(
        !text.contains("перший OPEN `#262`") && !text.contains("first OPEN `#262`"),
        "NEXT_PROBLEM must not keep #262 as first-OPEN"
    );
}
...
fn phase_o_status_row() {
    ...
    assert!(status.contains("| #262 |") || status.contains("#262"));
    assert!(status.contains("RFC-0153") || status.contains("0153"));
