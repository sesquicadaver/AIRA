//! Autostart hook tests (QUEUE #78 Linux / #87 macOS / #91 Windows).

use aira_desktop_runtime::{
    autostart_desktop_entry, is_autostart_enabled_in, is_launch_agent_enabled_in,
    is_windows_startup_enabled_in, launch_agent_plist, launch_agents_dir_for_home,
    set_autostart_in, set_launch_agent_in, set_windows_startup_in,
    windows_startup_dir_for_app_data, AIRA_AUTOSTART_FILENAME, AIRA_LAUNCH_AGENT_FILENAME,
    AIRA_LAUNCH_AGENT_LABEL, AIRA_WINDOWS_STARTUP_FILENAME,
};
...
#[test]
fn windows_startup_roundtrip_under_app_data() {
    let tmp = tempfile::tempdir().unwrap();
