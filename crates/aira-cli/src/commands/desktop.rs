...
use aira_desktop_runtime::{
    export_invite_file, export_invite_qr_png, import_invite_file, import_invite_qr_file,
...
pub(crate) fn run(command: DesktopCommands) -> Result<ExitCode> {
    match command {
        ...
        DesktopCommands::InviteExportQr {
            data_root,
            out,
            addr,
        } => {
            let paths = resolve_paths(data_root);
            let invite = export_invite_qr_png(&paths, &out, addr)?;
            println!("exported {}", out.display());
            println!("identity_ref {}", invite.identity_ref);
            if let Some(a) = invite.addr.as_ref() {
                println!("addr {a}");
        ...
        DesktopCommands::InviteImportQr { data_root, file } => {
            let paths = resolve_paths(data_root);
            let out = import_invite_qr_file(&paths, &file)?;
            println!("imported {}", file.display());
            println!("trusted {}", out.identity_ref);
            if out.book_updated {
                if let Some(a) = out.addr.as_ref() {
