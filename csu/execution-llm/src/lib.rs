...
pub use process::{
    ...
    CHILD_ENV_ALLOWLIST, DEFAULT_OLLAMA_HOST, EMPTY_STDOUT, ENV_LLM_BACKEND, ENV_LLM_LANDLOCK,
    ENV_LLM_NETNS, ENV_LLM_SANDBOX_REQUIRED, ENV_LLM_SECCOMP, ENV_OLLAMA_HOST_CFG,
    ENV_PROCESS_ARGS, ENV_PROCESS_BIN, ENV_PROCESS_TIMEOUT_MS, HOST_OLLAMA_BINDING_REQUIRED,
    LANDLOCK_FAILED, LANDLOCK_UNSUPPORTED, MISSING_BINARY, NETNS_BLOCKS_LOOPBACK, NETNS_FAILED,
    NETNS_UNSUPPORTED, NETWORK_NONE_CONTRACT, NONZERO_EXIT, OLLAMA_HOST_ENV, PIPE_OVERFLOW,
    PIPE_STDERR_LIMIT, PIPE_STDOUT_LIMIT, PROCESS_BACKEND_ID, SANDBOX_REQUIRED,
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn landlock_denied_read_is_capsule_failed() {
        let dir = tempfile::tempdir().unwrap();
        ...
        std::fs::set_permissions(&script, perm).unwrap();
        let mut csu = ExecutionLlmCsu::new()
            .with_process_backend(ProcessBackend::new(&script).with_landlock())
            .with_activate_gate(AlwaysActivated);
        let mut log = MemoryEventLog::new();
        let mut store = CasArtifactStore::open(dir.path().join("arts")).unwrap();
        let cap = bind_capsule(&mut store, &valid_generate_body());
    ...
    #[test]
    fn ollama_netns_is_capsule_failed() {
        let mut csu = ExecutionLlmCsu::new()
            .with_process_backend(
                ProcessBackend::ollama("aira-llm-process-missing-bin-215-do-not-install", "m")
                    .with_netns(),
            )
            .with_activate_gate(AlwaysActivated);
        let mut log = MemoryEventLog::new();
        ...
        assert!(outs.iter().any(|o| matches!(
            o,
            CsuOutput::Failure { message } if message.contains(NETNS_BLOCKS_LOOPBACK)
        )));
    }

    #[cfg(target_os = "linux")]
    #[test]
    fn netns_isolated_connect_is_capsule_failed() {
        let dir = tempfile::tempdir().unwrap();
        let bin = crate::process::compile_connect_probe(dir.path());
